...
export interface Engine {
	...
	cleanup: () => void
	successPayload: (ctx: { logger: { warn: (msg: string) => void; flush: () => string[] }; graph?: import('../types.ts').PackerContext['graph'] }) => Record<string, unknown>
...
export function defineEngine(overrides: EngineOverrides): Engine {
	...
	return {
		...
		successPayload: ({ graph }) => ({ dependencyGraph: (graph ?? getDependencyGraph()).toJSON() }),  // F24/F27：收 ctx，默认含 dependencyGraph（不用 logger）
