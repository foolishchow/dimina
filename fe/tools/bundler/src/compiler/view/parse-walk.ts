 ...
 *
 * D-PW-2：从 index.ts 真抽出。index.ts 只保留 compileML 编排 + viewEngine。
 * W1 两套 cycle-break shim：index.ts import 后调 bindVueToolsLive + bindTransformOrchestrator 注入。
 */
...
import { getAppId, getComponent, getContentByPath, getDependencyGraph, getTargetPath, getViewScriptExts, getViewScriptTags, getWorkPath } from '../core/env.ts'
...
export function parseSafeBraceExp(exp: string): string {
	return addOptionalChaining(parseBraceExp(exp))
}
export function transformTextInterpolation(text: string): string {
 ...
 */
export function initWxsFilePathMap(workPath: string): void {
	// 清空现有映射
 ...
 */
export function parseKeyExpression(exp: string, itemName = 'item', indexName = 'index'): string {
	// 去除首尾空格
	exp = exp.trim()
 ...
 * @param {*} str
 */
export function isWrappedByBraces(str: string): boolean {
	return /\{\{.*\}\}/.test(str)
}
export function splitWithBraces(str: string): string[] {
	const result = []
	let temp = ''
	let inBraces = false
...
export function parseClassRules(cssRule: string): string {
 ...
 * @param {*} attrs
 */
export function getForItemName(attrs: Record<string, unknown>): string {
 ...
 * @param {*} attrs
 */
export function getForIndexName(attrs: Record<string, unknown>): string {
	for (const key in attrs) {
 ...
 * @param {*} attrs
 */
export function parseForExp(exp: string, attrs: Record<string, unknown>): string {
 ...
 * @param {*} exp
 */
export function parseBraceExp(exp: string): string {
 ...
 */
export function parseTemplateDataExp(exp: string): string {
 ...
 */
export function ensureWxsScan(workPath: string): void {
...
/**
 * 重置 WXS 扫描路径（viewCompile 编译循环前调用，强制下次 compileML 重扫）。
 */
export function resetWxsScan(): void {
