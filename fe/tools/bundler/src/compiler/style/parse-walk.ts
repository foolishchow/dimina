...
import { getAppId, getComponent, getContentByPath, getDependencyGraph, getStyleExts, getTargetPath, getWorkPath } from '../core/env.ts'
...
/**
 * 清理 style parse+walk 缓存（styleCompile cleanup 调用）。
 */
...
export function boostExternalClassSelectors(cssCode: string, moduleId: string): string {
	if (!moduleId || !cssCode) {
		return cssCode
	}
 ...
 */
export function ensureImportSemicolons(css: string): string {
 ...
 */
export function processHostSelector(selector: string, moduleId: string): string {
