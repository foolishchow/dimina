...
[![npm version](https://img.shields.io/npm/v/@dimina/bundler.svg?style=flat)](https://www.npmjs.com/package/@dimina/bundler)
...
```sh
npm install @dimina/bundler -g
```

或者在项目中本地安装：

```sh
npm install @dimina/bundler --save-dev
```
...
{
  "scripts": {
    "build": "dimina-cli build",
    "dev": "dimina-cli build -w"
  }
}
```
...
dimina-cli build -c ./src -s ./dist

# 监听文件变化，实时编译
dimina-cli build -w

# 完整示例：编译指定目录，输出到指定目录，并启用监听
dimina-cli build -c ./src -s ./dist -w

# 产物根目录不包含appId
dimina-cli build --no-app-id-dir

# 生成 Harmony QuickJS 断点调试所需的 source map
dimina-cli build -c ./miniapp -s ./dist --sourcemap
```
...
```sh
# 内存紧张或希望降低 CPU 峰值
DIMINA_COMPILER_MAX_WORKERS=1 dimina-cli build

# 吞吐优先的高配构建机
DIMINA_COMPILER_MAX_WORKERS=3 dimina-cli build

# 单个 Worker 的 V8 old generation 上限（MiB，默认最多 2048）
DIMINA_COMPILER_WORKER_MEMORY_MB=1024 dimina-cli build
```
...
```js
import build from '@dimina/bundler'
