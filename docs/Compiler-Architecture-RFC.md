...
| 关联文档 | [架构总览](./Architecture-Diagram.md) · [实现细节](./Architecture-Details.md) · [维护经验](./Experience-Review.md) |
...
- **G3 统一挂载面（必达）**：编译器生命周期可挂载——dev server、宿主插件、未来 lynx adapter 通过同一套 hook / target / 插件契约接入，不再向任务树塞分支。
- **G4 产物契约护栏（全程有效）**：改造期间产物字节级不变，57 个既有 spec 全绿；这是约束而非交付物。
...
- **运行时语义修改**：不触碰四端容器行为、bridge 契约与 service/render 运行时语义。
...
**后果**：`fe/packages/container` 角色逐步收敛为参考宿主；compiler 对 container-sdk 的依赖方式（peer / 预构建宿主页随包分发）在 A2 落地时定案。
...
**理由**：dev/HMR/统一三个目标用纯 JS 即可达成（A 轨道）。Rust 宿主的价值在「统一核心 + 阶段黑盒契约 + 与 lynx/rspeedy 工具链同构」，并顺带消除 napi 边界序列化（AST 全程留在 Rust 内存）与 JS 侧字符串热点；oxc 核心能力均有 Rust crate（`oxc_parser`/`oxc_ast`/`oxc_walker`/`oxc_codegen`/`oxc_minifier`/`oxc_resolver`/`oxc_sourcemap`/`oxc_transformer`），Rust core 约等于「把 oxc 从 napi 调用换成 cargo 依赖，再补领域代码」。**两条轨道解耦**：A 轨道定义阶段接口与插件契约，B 轨道在接口之内替换实现，互不阻塞。
...
**理由**：四端容器、运行时与既有回归测试共同锁定的行为边界。改造期间 `modDefine` 格式、模块 ID 规则、输出目录结构（`main/`、`{root}/`、`app-config.json`）、兼容性警告语义一个字节不变。

**后果**：57 个既有 spec 全程作为行为护栏；新增能力（hook、dev、target）的测试按 [维护经验](./Experience-Review.md) 第 6 条执行消融实验。
...
| 阶段 | 内容 | 验收标准 |
| --- | --- | --- |
| **A1 hook 层** | 把 Listr 任务树抽为可挂载生命周期（env → config → npm → [view ‖ logic ‖ style] → publish），不改任何行为 | 57 测试全绿；`build()` 公开行为与错误契约不变（`build-error-contract.spec.js` 重点回归） |
| **A2 dev server + L1** | `dmcc dev`：静态服务 + 内置宿主页 + 代理 + ws；logic 变更 → 页面 relaunch | 示例 app 一条命令起预览；§4.2 L1 场景验收 |
| **A3 HMR L2/L3** | CSS 热替换 + 模板热重挂（render/container-sdk 侧配合） | §4.2 L2/L3 场景验收；编译失败不中断运行实例（消融：注入失败用例验证旧产物保留） |
...
| **B1 sourcemap 试点** | `oxc_sourcemap` 替换 source-map-js 合并逻辑 | sourcemap 三组 spec 全绿；Harmony QuickJS attach 实机验证；耗时 ≥ 基线 |
...
| **C1 lynx PoC（可选，另立 RFC）** | `view/text/image` 三组件单页面走通 `.lyx` 链路 | bridge 链路图（事件产生→渲染完成）先行；Android demo 实机验收 |
...
| 编译失败打断运行中实例 | 高 | 发布目录原子性（编译成功才 swap）；失败保留旧产物 + 明确提示（见 §4.2 验收） |
