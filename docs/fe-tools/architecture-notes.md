...
**落点权威**：目录/文件级 Packer / Scheme / 焊点归属见 [`fe-tools-bundler-boundaries`](../actions/_archive/complete/fe-tools-bundler-boundaries/technical-design.md) §2 落点表（D-BD-1..6）。4 个焊点：`packer/emit/emit.ts`、`compiler/logic/**`、`packer/store/env.ts`、`packer/graph/dependency-graph.ts`（目录收敛 fe-tools-packer-directory-convergence 后路径，见下）。本术语节不展开焊点拆分；方法级拆分另立。
...
**H3 Phase 2 — per-module 选择性重编译（fe-tools-hmr-per-module-cache Phase 2 · 2026-10-09）**：H3 归档时 deferred 的 D-PMC-2 全量实施（F-H2-1 view L/C/E 拆分完成后）。`ViewSelectContext{viewCache, dirtySet}`——compileModule 加 ★ selective skip 路径：clean + cached module → load 仍执行（instruction 供父级 wxs 聚合 + templateModule 供继承链）+ seed scriptRes from cache（module code 从 viewCache；wxs entries 从 instruction.scriptModule——字节同源）→ 跳过 compile。**递归编排不动**（compileViewTree 的 inherited sets + page 双编译 + wxs 聚合全保留——select 只是 per-module compile 过滤器；scriptRes 插入序 = EmitModule[] 序 = order list 序）。compileML 三分支：cache-hit（全 cached）→ **selective**（orderList 存在 + 全 module cached∪dirty → 只重编 dirty）→ 全量 miss。pageBundles 只返 dirty 子集（IPC 经济——HMR 热路径单 module 变更只传变更 module）+ selective flag + **orderList 全量回传**（stale-orderList 防护：wxs 集变更时新 id 须入序，否则下轮 cache-hit assemble 缺 module）。**page 渲染独立于 component 代码**（bundle = module concat）→ clean page + dirty component → page 复用 cached 无需重编。行为 0：tsc 0 + vitest 644/644（4 新 view-selective-recompile tests：component dirty 字节恒等 + page dirty + cache 更新后 hit 复用 + wxs 集变更 stale-orderList 回归）+ 6 项目 diff=0 + V-PC-5 0。测试教训：① component moduleId 带斜杠（'/components/mycomp/index'——usingComponents 值）vs page 不带（'pages/home/index'——app.json）；② 测试 helper 复制的低级 typo（return p 目录 vs return r 文件）→ EISDIR。

**HMR-compiler 伞收口（fe-tools-hmr-compiler · complete 2026-10-09）**

**HMR 链 residuals 收口（fe-tools-hmr-chain-residuals · complete 2026-10-09）**：HMR-compiler 伞复盘发现的"代码在/接线断/证据缺"三档残留逐项消解。**交付**（行为 0：tsc 0 + vitest 87/646 + 6 项目 diff=0）：① R-HR-5 COMPILE_STAGE_ORDER 定义迁 model（`src/model/stage-order.ts`，③a 消解——`invalidation.ts:69` import model，model→pipeline import=0）；② R-HR-2 L_HMR flag-gated 通道补齐（`--hmr`/`DMCC_HMR` → preview-adapter `enableHmr`，默认 false=今日，runtime 就绪后翻 flag）；③ R-HR-4 StageChannelContext typed 边界（`packer/types.ts` 16 字段，stage-channel 12 处 + orchestrator 20 处 `ctx as { field }` 收敛为 `sctx.field`，grep `ctx as {`=0）；④ R-HR-3 selective 链路级证据（`view-selective-stages.spec.js` build() 全链：selective 触发+dirty 子集+字节恒等）；⑤ D-HR-1 registry 接线（locked B minimal）：CompileRegistryImpl/EmitRegistryImpl 实体化（替代 stub）+ orchestrator load stage `loaderRegistry.kinds()/get()` 生产消费点 + `ctx.loadedModules`。**Residual（deferred 后续门）**：R-HR-1 (b)/(c) view/style Loader 注册 + 阶段函数注册 blocked——预研发现 view/style 阶段函数需 page/继承上下文，不 fit 单 module Loader/Compiler 接口，全量注册须 types.ts 接口演进（LoadedModule/Compiler 形状变更）= Non-scope。③b compile-cache.ts:5（runtime residual）/ ③c convergence.ts:3（type-only，runtime 无害）open。F-HR-2 默认 true deferred（runtime HMR API 就绪）。：编译侧 HMR 全链交付完毕。子门 H1（deriveFromGraph 接线）+ H2（registry 实体化 + Phase 2 F-H2-1 L/C/E 拆分）+ H3（per-module cache + Phase 2 选择性重编译）+ H4（per-module push + Phase 2 publish 增量 sync）全 complete。**R-HMR-1..5（MUST）全 pass**；R-HMR-6（SHOULD）partial：0 runtime cycle ✓（① env→packer 单向 runtime、② pipeline/emit 无 domain import），残留 ① env.ts→packer upward leak（W3 gradual migration 在档）+ ③ model→pipeline（D-REG-3 deferred COMPILE_STAGE_ORDER 下沉）——SHOULD 不阻塞。**伞外依赖**：runtime HMR API（运行时侧——D-HMR-5 非阻塞；编译侧 enableHmr 就绪，runtime 就绪后激活 L_HMR per-module hot-swap）+ compile/emit registry 接口注册（阶段函数已就绪——logicLoader 已注册，C/E 管线接线是后续 Action 域）。行为 0 最终态：tsc 0 + vitest 86 files/644 tests + 6 项目 diff=0（跨 H2p2+H4p2+H3p2 累积验证）。

**packer 域目录收敛（fe-tools-packer-directory-convergence · complete 2026-10-09）**：packer 域 ~27 文件散在 4 处（`packer/` + `model/` + `compiler/pipeline/` + `compiler/worker-runtime/`）+ `compiler/core/` 混合袋 → 归位 `packer/` 9 子目录（mirror 北星 6 组件形状：graph/store/registry/state/cache/emit/worker/pipeline/aspect）。**纯搬迁无逻辑改**（D-DC-2 红线，函数体逐字不动；registry.ts 拆 dispatch.ts+lce.ts 逐字搬迁，R10-2 验零互调）。**分批 B1a→B5**（每批行为 0 gate）：B1a graph/（graph+config-fixpoint+dependency-graph+npm-resolver）→ B1b store/（env+project-store，env stateful crux，57 导入方原子）→ B2 cache/+registry/（4 cache + registry 拆分 + renderers stateful）→ B3 emit/+worker/（6+7 文件，③c 消解 convergence→emit 同子目录；executor ENTRY_PATH runtime 路径修正）→ B4 pipeline/+state/（stage-order 锁 pipeline/ per F-R5-2；③b 消解 compile-cache→compile-stages intra-packer）→ B5 aspect/+core 解体（compatibility stateful + sourcemap/expression-parser/compatibility-reference 去 shared + npm-builder→pipeline；compiler/core/ 删）。**③b/③c 消解**：跨顶层目录 import 全收敛为 intra-packer（grep `from '.*compiler/pipeline' src/packer/` = 0）。**stateful 原子性红线**（D-DC-3）：env/compatibility/renderers 单 commit 原子（全 static + 全 dynamic import() 字符串 + 删旧），dual-instance 陷阱防（P-DC7b 单实例 grep = 0）。**方向单向**（D-DC-4）：packer→compiler 17 import（I/O 供，compiler 不反向 import packer 内部）。**终态**：`compiler/` 只剩 `logic/view/style`（per-kind 不动 D-DC-5）；`model/` + `compiler/pipeline/` + `compiler/worker-runtime/` + `compiler/core/` 全删。行为 0：tsc 0 + vitest 87/647（compile-cli-cache flaky solo pass）+ 7 项目 diff=0（air-battle/base/mpx-demo/subpackages/taro-todo/vant/weui）。3 批 21 轮 review 严格收敛（R8+R9/R17+R18/R20+R21 三组连续 0）。**bridge**：本 Action 只做目录收敛（结构 Round 0），给后续 D（facade 收敛）+ C（aspect 分离）轮干净素材；registry.ts 拆分是形状建立第一步（dispatch vs lce 层次分离，F-PA-5 收口开端）。
**北星 interface 演进（fe-tools-packer-north-star-evolution · complete 2026-10-10）**：D-OR-7 三重张力消解。D-NS-1 Graph 5 accessors（getAppId/getAppName/getAppConfigInfo/getConfigData/getPageConfigInfo）+ PageConfig relocate（getPageConfigInfo 从 configInfo Proxy 迁 graph）；D-NS-2 ModuleResultCache<V> 泛化 + get size；D-NS-3 BuildResult composite（config/graph/emitResult/cast 三组）；D-NS-4 PackerOrchestrator : 注解（bivariance）；D-NS-5 CompileOptions/WatchOptions/CompileRequest/WatchRequest 收敛；D-NS-6 packerALS + runWithCompilerContext 删（dead）。**backflow**：storeInfo compat 写经 audit load-bearing RETAINED（主线程 getter 消费方未全迁 storeInfo() 返回值）——推迟为后续 initiative（R-NS8）。行为 0（tsc 0 + 648 pass + 7 diff=0）。

**Output 抽象（fe-tools-packer-output-abstraction · complete 2026-10-10）**：抽象统一 Output interface（add/read/publish/getEntries）+ MemOutput（dev memfs）+ DiskOutput（one-shot disk）——替代 BuildModel/materialize/publishToDist/createDist/artifactResolver/skipMaterialize 10+ 概念缠结。消 targetPath 双语义（DiskOutput 封装 scratch→final）+ compat 写 output-path 消费方死（为 storeInfo 塌缩铺路）。D-O1..7 + D-OL1..4 locked（19 轮 review）。P-O1-O3 全实施 complete。行为 0（tsc 0 + 648 pass + 7 diff=0）。

**storeInfo 塌缩（fe-tools-storeinfo-collapse · complete 2026-10-10）**：消除 storeInfo 矛盾身份（消耗品 + singleton mutator）+ sctx.storeInfo 冗余投影——新增 `storeInfoCtx(ctx, graph, state) → void`（orchestrate 链路纯函数，调旧 storeInfo 取 pathInfo → `state.scratch`）。concept 3→2（PackerContext + state.graph）。**PackerSessionState.scratch**（TEMP mkdtemp per-orchestrate，mutable）+ orchestrator `tasks.run({output, ctx, state})` 注入 sctx.ctx/sctx.state + StageChannelContext 加 ctx?/state?。**collaborator 全迁** sctx.ctx/sctx.state.scratch（config-compiler-collab/stage-dispatcher/npm-builder/publisher/dist-preparer/logic-emitter/config-collector）——`compileConfig` 收 `state.scratch`（非 ctx.targetPath FINAL）。**buildResetStoreInfoData helper**（env.ts export）统一 3 处 worker resetStoreInfo（logic emit-engine + view/style stage-channel L45）——字段名转换 directivePrefixes→templateDirectivePrefixes + configInfo `as ConfigInfo`。**殁骸清**：删 StageChannelContext.storeInfo + sctx.storeInfo 赋值（caller=0）+ ProjectStore interface 删 getDependencyGraph/merge/snapshot（config-collector 改 state.graph.getInnerGraph()）。**compat 写保留 backflow**：旧 storeInfo 不动（compat 写 load-bearing——105 测试调用点依赖 ALS getter getTemplateExts/getWorkPath 等，删则崩；与 D-NS6 R-NS8 backflow 一致），推迟为后续 initiative（阶段 2 L1 迁出 + 阶段 3 worker ALS 退役）。D-SC1..6 locked（scratch 流方案 C state.scratch + PackerContext 流 sctx.ctx/sctx.state + buildResetStoreInfoData helper 3 处 worker + getDependencyGraph 退役 + compat 写 backflow）。12 轮 review pass（文档 8 + 深层 4）+ close review 2 轮（F-C1..4 全修正）。行为 0（tsc 0 + vitest 88/648 flaky-solo + 7-diff=0）。
**env.ts L1 迁出（fe-tools-env-l1-extract · complete 2026-10-10）**：env.ts 三层混合（L1 计算 + L2 ALS 门面 + L3 worker 桥接）→ L1 迁纯模块 `src/packer/store/env-compute.ts`，env.ts 退化为 L2/L3 薄门面 + storeInfo wrapper（compat 写保留 backflow）+ re-export L1（方案 A 最小改动——消费方 import from env.ts 不变）。**D-EL1-1..6 locked**（11 轮 readiness review，19 findings 全修正）：computeStoreInfo 5 字段返回（含 npmResolver——喂 9 测试文件主线程 parse-walk 链 F-R6-1，R2 实证盲区修正）+ CompilerContext/toPackerContext export（实施期 deviation：原 internal 修正为 export——env.ts getPages 薄壳 21 文件 47 fixture 依赖须引用）+ 迁移细则（SC_TRACE x2 + 顺序双段逐字搬迁 F-R4-1）+ resolveAppAlias 选项 A 锁定（env.ts wrapper 读 ALS）+ 死代码清理 5 函数（storeProjectConfig/storeAppConfig/storePageConfig/createInitialDependencyGraph/storePathInfo）+ toPackerContext/PageConfig-ComponentConfig re-export/getCompilerContext export 删。getPages 保留 env.ts L2（21 文件 47 调用点）。env.spec.js 改写 config-fixpoint.readProjectConfig 直测（保留合并覆盖）。行为 0（tsc 0 + vitest 88/648 flaky-solo + 7-diff=0）。backflow：compat 写保留（storeInfo wrapper——阶段 3 退役）+ L2/L3 不动（getters/resetStoreInfo 保留）+ scratch 内化 + PackerContext dedup 留 follow-up。
- 2026-10-10 **fe-tools-scratch-internalize**（complete）：mkdtemp 从 storeInfo 链路（env-compute.computePathInfo）内化到 Output 层（BaseOutput 构造）。D-SI-1 BaseOutput protected constructor + 内联 mkdtemp（TARGET_PATH env/GITHUB_WORKSPACE/os.tmpdir/dimina-fe-dist-）+ scratch non-enumerable（Object.defineProperty——mkdtemp 随机路径不进 BuildResult.output 深对比，行为 0 纪律）；D-SI-2 orchestrator L166 后 state.scratch = output.scratch 投影（consumer 7 处不改读源）；D-SI-3 computeStoreInfo 收 pathInfo? 参数（storeInfoCtx 默认 {workPath} 无 targetPath；storeInfo wrapper 传 computePathInfo 含 targetPath）；D-SI-4 storeInfo compat wrapper 保留 mkdtemp（测试 fixture backflow——compile-cli-cache 唯一性测试）；D-SI-5 dev 须 mkdtemp（MemOutput 也 mkdtemp——createDist/compileConfig/npm-builder 防崩；dev server 读 Output.read 内存不受影响）。mkdtemp 归属正位（I/O 层拥有 TEMP 生命周期）。10 轮 readiness review 收敛（R9+R10 连续 0-finding）。行为 0（tsc 0 + vitest 88/648 flaky + 7-diff=0）。backflow：compat 写保留（storeInfo wrapper mkdtemp）+ PackerContext dedup（buildPackerContext/buildFixpointCtx/toPackerContext 三同质）+ L2/L3 退役（阶段 3）。
- 2026-10-10 **fe-tools-packer-context-dedup**（complete）：PackerContext 构造逻辑 dedup（三处逐字重复收敛单一内核）。D-PCD-1 抽 buildPackerContextFromOptions 内核（config-fixpoint——收已 normalized compilerOptions + readContent/resolveAlias/resolveNpm stub 逐字搬迁 + fileTypes 字段名映射 templateDirectivePrefixes → directivePrefixes 统一 + 无须 as PackerFileTypes 断言）；D-PCD-2 buildPackerContext = normalize + 内核；D-PCD-3 toPackerContext = 取 CompilerContext 字段 + 内核；D-PCD-4 buildFixpointCtx = 内核 + 包 FixpointCtx（npm 独有）；D-PCD-5 内核放置选项 A 锁定（config-fixpoint graph 层——store→graph 单向已存在，无循环）；D-PCD-6 字段名映射统一 + config-collector 反向解构保留。承接 scratch-internalize + env-l1 backflow（PackerContext 构造同质化）。11 轮 readiness review 收敛（R6+R7 + R10+R11 双重连续 0-finding）。行为 0（tsc 0 + vitest 88/648 flaky + 7-diff=0）。无 implementation deviation。backflow：阶段 3 L2/L3 退役 + compat 写保留 + resolveAlias/resolveNpm stub 实体化（D-PCS-1）。
- 2026-10-10 **fe-tools-l2-l3-retire-research**（complete，研究性）：L2+L3 退役大倡议 source-audit + 拆分方案（不实施代码）。audit L2 ALS getters（15 × compiler/* 10 文件）+ L3 resetStoreInfo（4 worker 引擎）+ compat 写 storeInfo wrapper（112 caller）+ 门控链条（L2 退役 ← compiler/* 迁 PackerContext ← worker ctx 直传 ← PackerContext 序列化 readContent 不可序列化）。拆分 A0（worker ctx 直传——根门控，方案 b 锁定：storeInfo→ctx data + buildPackerContext 重建）+ A1 logic/A2 view/A3 style parse-walk 迁移（并行，门控 A0；A1-A3 各 ~15-16 处 getter 替代 ctx）+ A4 compat 写退役（门控 A1-A3）+ A5 singleton/Proxy 退役（门控 A1-A4，getPages 27 caller 特殊门控）。双重收敛（readiness R6+R7 + 深度 R13+R14 连续 0-finding）。无代码改动（git diff=0）。backflow：A0-A5 各自 formalize 独立 Action。
- 2026-10-10 **fe-tools-worker-ctx-direct**（complete）：L2+L3 退役第一步实施（A0+A1 合并——worker ctx 直传 + logic parse-walk 迁移）。logicCompile 建 ctx（buildPackerContextFromOptions from storeInfo data）+ compileJS/buildJSByPath 透传 ctx（optional + fallback ALS——34 测试直调 compileJS 保留 compat）+ logicParseWalk 加 ctx 第 8 参（optional + fallback ALS）+ registry-impl _ctx→ctx（optional）。ctx 读 3 getter（getWorkPath/getTargetPath/getContentByPath→ctx.readContent）+ 保留 ALS 7 getter（getDependencyGraph/getAppId/getNpmResolver/resolveAppAlias/getAppConfigInfo/getComponent/isMiniGame——A5 统一迁）。**resolveAppAlias 行为 0 守护**（ctx.resolveAlias stub 返 null ≠ ALS resolveAppAliasImpl 实际解析——保留 ALS）。4 deviations（ctx optional + 末尾位置 + DRY 提前 + registry-impl 测试不传 ctx）。行为 0：tsc 0 + vitest 88/88 + 7-diff=0。双重收敛（readiness R6+R7 + 深度 R11+R12 连续 0-finding）。
- 2026-10-10 **fe-tools-singleton-retire-impl-core**（complete）：A5a 核心 migration——PackerContext 扩 6 optional（graph/appId/component/configInfo/npmResolver/runtimeType + resolveAlias 闭包 appInfo）+ ALS 拋留 31 处迁 ctx 读（ctx optional + fallback ALS 渐进）+ successPayload 签名扩 { logger, graph? } + view ctx 透传链补全（A2 遗留——viewCompile 建 ctx + compileML 透传 + viewParseWalk caller 传 ctx）。types.ts PackerContext 扩 6 optional（D-PCS-1/D-PCS-6 放宽——graph optional 加到 PackerContext）+ import DependencyGraph/NpmResolver。config-fixpoint.ts buildPackerContextFromOptions 扩 optional 参数（graph/appId/component/configInfo/npmResolver/runtimeType/appInfo）+ resolveAlias 闭包 appInfo（A0 R8 守护——appInfo 传实体化，不传 fallback stub）。logic/parse-walk logicParseWalk 加 _graph helper + 5 处 getDependencyGraph→_graph + getAppId→ctx.appId fallback。logic/index.ts compileJS/buildJSByPath ALS getter 迁 ctx 读 + logicSuccessPayload 签名扩 graph? + fallback ALS + worker ctx 建立传 graph + import MINI_GAME_RUNTIME_TYPE。view/parse-walk compileViewTree 加 _graph helper + 6 处 ALS getter 迁 ctx 读。view/index.ts viewCompile 建 ctx + compileML 加 ctx 参数 + viewParseWalk caller 传 ctx + viewSuccessPayload 签名扩 + fallback ALS + import buildPackerContextFromOptions。style/parse-walk enhanceCSS/normalizeCssUrlValue 3 处 ALS getter 迁 ctx 读。style/index.ts worker ctx 建立传 graph。define-engine.ts successPayload 签名扩 { logger, graph? } + 默认 fallback ALS。env.ts getDependencyGraph 加显式返回类型 :DependencyGraph（TS 推断 void quirks）。4 deviations（ctx.graph 类型 DependencyGraph + getDependencyGraph 显式返回类型 + helper 变量 _graph + successPayload runtime caller fallback ALS）。行为 0：tsc 0 + vitest 88/88 + 7-diff=0。双重收敛（readiness R6+R7 + 深度 R10+R11+R12 三重连续 0-finding）。
- 2026-10-10 **fe-tools-singleton-retire-impl-cleanup**（complete，partial）：A5b cleanup partial——D-SRC-1a + D-SRC-3a 完成（ctx 传全核心 + 测试传 ctx），D-SRC-1b/2/3b 完全退役收尾推迟后续 action（fe-tools-singleton-retire-cleanup-final）。D-SRC-1a worker ctx 建立传全 optional（graph/appId/component/configInfo/npmResolver/runtimeType/appInfo——logic/view/style index.ts）+ view 补传 graph（A5a 遗漏）+ runtime caller 传 graph（候选 a 过渡——import getDependencyGraph + successPayload({ logger, graph: getDependencyGraph() })）+ import 补全（logic +3/view +5/style +多）。D-SRC-3a 测试迁 15 文件 compileSS/compileML 传 ctx + storeInfo si/ctx 建立 + buildCtxFromStoreInfo helper（getter try/catch fallback——测试 fixture 缺 project.config.json 等）+ view-style-cache-skip/module-cache ctx 作用域 fix（describe 级 let ctx）+ custom-file-types store 别名逐行处理。D-SRC-1b 删 fallback 15 处破坏独立函数（enhanceCSS/collectAllWxsModules/styleLoad caller 不传 ctx，121 failed）——完全退役收尾须迁独立函数 10 处 + caller 链 + 测试直调，scope 极大 + 行为 0 风险高，推迟后续 action。完全退役不改变行为（ctx 已携带全 data，fallback ALS 仅保险——ctx 优先 ctx?.x ?? ALSGetter()）。21 轮 review 收敛（readiness R6+R7 + 深度 R10+R11+R12 + 额外 R15+R16+R17 三重连续 0-finding）。行为 0：tsc 0 + vitest 88/88（3 flaky solo pass）+ 7-diff=0。backflow：D-SRC-1b/2/3b 完全退役收尾推迟（fe-tools-singleton-retire-cleanup-final）——删 fallback + 独立函数迁 + resetStoreInfo 退役 + env.ts singleton 删 + src getter caller 迁 + runtime 改候选 b。
- 2026-10-10 **fe-tools-singleton-retire-cleanup-final**（complete，partial）：A5b 推迟的 D-SRC-1b/2/3b 完全退役收尾——D-SCF-1-1 logic parse-walk 完成（partial），view/style + 1-2/2/3b 推迟。D-SCF-1-1 logic parse-walk 6 函数加 ctx optional（getJSAbsolutePath/resolveDependencyId/resolveNpmModuleId/resolveModuleIdToExistingPath/resolveRelativeModuleId/resolveBareSiblingModuleId）+ caller 链传 ctx（parse-walk 内 + logic/index.ts:122 + registry-impl.ts:24）+ fallback ALS 保留（ctx?.x ?? ALSGetter()——行为 0，ctx optional 不破坏）。resolveNpmModuleId resolveScriptModule function ref 用 wrapper (id) => resolveModuleIdToExistingPath(id, ctx)。**推迟根因**：view parse-walk 递归 caller 链（compileModule → tryModuleCache → mergeWxsModules → collectAllWxsModules——每加 ctx 到一个函数，caller 在另一个无 ctx 函数内）+ A2 deviation D-VPM-dev1 compileModule 撤回 ctx 阻塞（compileModule 不加 ctx，但 tryModuleCache/mergeWxsModules/collectAllWxsModules 须 ctx——矛盾）+ scope 极大（删 fallback 几十处 + resetStoreInfo 退役 + emit.ts:142 + emit-engine 建 ctx + env.ts singleton 删 + 几十处 caller + runtime 改候选 b）。完全退役不改变行为（ctx 已携带全 data，fallback ALS 仅保险）。14 轮 review 收敛（readiness R6+R7 + 深度 R13+R14 双重连续 0-finding）。行为 0：tsc 0 + vitest 88/88（3 flaky solo pass）+ 7-diff=0。backflow：view/style parse-walk + D-SCF-1-2/2/3b 推迟（须撤回 A2 deviation compileModule 加 ctx + 递归 caller 链 + wxml/load/index.ts 透传链 + style parse-walk）。
