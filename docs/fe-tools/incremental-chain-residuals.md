...
| R3 | low | open | 9-24 F-R3 | ctx 类型三分（`as { viewCache? }` 结构断言）+ cache 三套（ModuleResultCache / view bundle Map / style bare Map） | `orchestrator.ts:187` / `stage-channel.ts:49` | ~~HMR 前可接受~~ **条件已过期**（HMR complete 2026-10-09）——ctx 断言收敛归 [fe-tools-hmr-chain-residuals](../actions/fe-tools-hmr-chain-residuals/README.md) R-HR-4；cache 三套结构统一仍 residual |
...
| F-HR-1 | low | open | 10-09 HMR 血缘复盘 | loaderRegistry 注册零消费——`logicLoader` 注册后全仓无 `get/kinds` 生产调用（仅 types.ts:376 文档注释）；registry 是注册态非消费态 | `packer/orchestrator.ts:83-84` | [fe-tools-hmr-chain-residuals](../actions/fe-tools-hmr-chain-residuals/README.md) R-HR-1（D-HR-1 接线） |
| F-HR-2 | info | open | 10-09 HMR 血缘复盘 | L_HMR 无生产合成路径——`enableHmr` 参数存在但 `preview-adapter.ts:43` 不传（恒 false）；与 D-PUSH-2 locked 选项②（编译侧发 L_HMR）接线不符 | `dev/dev-reload.ts:40` / `session/preview-adapter.ts:43` | [fe-tools-hmr-chain-residuals](../actions/fe-tools-hmr-chain-residuals/README.md) R-HR-2（D-HR-2 flag-gated） |
| F-HR-3 | low | open | 10-09 HMR 血缘复盘 | selective recompile 端到端零证据——接线在（invalidatedModules → worker → dirtySet），仅 4 个 compileML 直调集成测试；watch 链路级未验 | `view/index.ts:122-133` / `stage-channel.ts:84-92` | [fe-tools-hmr-chain-residuals](../actions/fe-tools-hmr-chain-residuals/README.md) R-HR-3（D-HR-3 边界级测试） |
| ③a | low | open | HMR 伞 A-HMR6 残留 | model→pipeline leak 分支 1：`COMPILE_STAGE_ORDER` from `compile-target.ts`（D-REG-3 deferred 项） | `model/invalidation.ts:69` | [fe-tools-hmr-chain-residuals](../actions/fe-tools-hmr-chain-residuals/README.md) R-HR-5（本 Action 消解） |
| ③b | low | open | F-R1-1 review 发现 | model→pipeline leak 分支 2（runtime）：`getCompileStagesForFiles` from `compile-stages.ts`——③ 全量消解须 registry dispatch 深化 | `model/compile-cache.ts:5` | residual——D-HR-1 后续门评（stage 概念归位依赖 dispatch 程度） |
| ③c | info | open | F-R2-2 review 发现 | model→pipeline leak 分支 3（type-only，erased）：`import type { EmitModule }` from `emit.ts`——runtime 无害但概念属 ③（emit 概念在 model） | `model/convergence.ts:3` | residual——type-only 不阻塞；若 ③ 全量消解则 type 迁移 |
