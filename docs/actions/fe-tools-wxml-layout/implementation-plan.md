...
Status: **`in_progress`** — L0→L1→L2；行为 0。
...
5. **renderer/**：registry/stub + `vue/{index,tools}.js`（tools 可含原 live/state）；按 §4 全库替换（含 `vueBackend`→`vueWxmlRenderer`、`createStubBackend`→`createStubWxmlRenderer`、`WxmlBackend`→`WxmlRenderer`）；删 `backends/`。
...
| 门 | Gate |
| --- | --- |
| L0 | 树对照 §1；无生产路径 import 旧目录；无旧 Backend 符号残留 |
| L1 | P-WL05 绿 + P-WL06 diff=0 |
| L2 | architecture-notes 已更新 |
