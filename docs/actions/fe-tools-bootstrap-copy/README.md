...
- Draft review：#1 + #2（2026-09-10）→ **`ready`**。
...
| G4 | 改名清单 | 否 | Scope 表已有 |
| G5 | 实施授权 | 实施门 | **待授权** `in_progress` |
| G6 | validation 计划表 | 否 | 已有 |
| G7 | STATUS 合并策略 | 否 | 禁止盲覆盖 |
| G8 | 开分支前对齐 `origin/main` ← didi | 实施前置 | 操作项 |

**Verdict**：Readiness **pass** → `ready`。实施须另授权；先对齐 `origin/main`，再 tag / 开 `feature/fe-tools-bootstrap`。

## Closure conditions

- acceptance 项通过并有证据；  
- D-BC-1 门禁满足；  
- STATUS 本行更新为 `complete`（并按惯例归档）。
...
| [validation](validation.md) | 计划命令与证据（执行时填写） |
