...
Status: **`in_progress`（S0–S1 已交付，待 Close）**

## 基线与纪律

- 实施起点 HEAD：`8b024b78`（validation Actual）；`fe/packages` 零触碰。
...
| 门 | Gate | Result |
| --- | --- | --- |
| S0 | typecheck 脚本 + fe-tests.yml 可跑且必过；tsconfig 符合 D-TC-1..4/7 | **pass** |
| S1 | 白名单 `@ts-check`；集中 typedef；`tsc --noEmit` 0 error；行为 0 / 消融 / 回流 | **pass**（580/580；diff=0；消融 ✓） |
