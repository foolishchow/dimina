...
- 选择点：**orchestrator `orchestrate()` 入口**（L121 request 构造后，mode-aware 点）创建 Output 设 sctx.output（经 task ctx 初始化）；config-collector 删 buildModel 行只消费 sctx.output。**F-R30-1 outputMode 信号**：P-O1/P-O2 用 request.skipMaterialize（boolean 过渡）；P-O3 后改 request.outputMode 'dev'|'disk'（enum 替代 boolean）
