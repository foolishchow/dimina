...
Status: **complete（2026-09-15）** — P-WX00/02/03/04/05/06/07 全 pass；P-WX01 **声明 Uncovered**（fork Actions 未启用，blocked-on-env——以 didi 侧回流 PR 运行为准）；证据见 Actual。

权威参考：[Experience-Review.md](../../../../Experience-Review.md)

| ID | Check | Command / method | Maps to | Result |
| --- | --- | --- | --- | --- |
| P-WX00 | build 跨平台 | `cd fe/tools/wxml-parser-napi && pnpm build`（napi CLI `--platform` 路线乙）；产物 `index.<platform>.node` + `binding.js`（ESM）就位且手写 `index.js` 未被覆盖；**P0 验 darwin（本地）+ linux（CI）**，win32 真验归 P1 matrix（**R1-F5 限定**） | A-WX0 | ✓（见 Actual） |
| P-WX01 | CI 绿 | push 后 fe-tests Actions 全绿；首次全量 Rust 编译分钟数记录；cache 命中增量对比（第二次 push） | A-WX0 | **Uncovered** — fork Actions 未启用（零 runs + dispatch 403）；声明以 didi 侧回流 PR 运行为准 |
| P-WX02 | 退 git + 最小加载 + dev 流 | `git ls-files` 无 `*.node`（含旧 `index.node` / `index.*.node`）；`.gitignore` 含 `*.node`；本地 `pnpm build` → 存在 `index.<platform>.node` 且 vitest 全绿（580）；移走该文件 → 编译测试炸（延迟抛错）→ 恢复绿 | A-WX0/A-WX1 | ✓（见 Actual） |
| P-WX03 | 子包结构 | `napi create-npm-dirs`/`prepublish` 产五子包；主包 `optionalDependencies` 声明；`private` 已除 | A-WX2 | ✓（见 Actual） |
| P-WX04 | 双态解析 | 单测三态（本地 `index.<platform>.node` 优先 / `binding.js` 子包检测路径 / 皆无抛错）——落 bundler `__tests__/wxml-parser-loader.spec.js`，经 `_resolveNative()` 可测钩（**R1-F6**）；有本地产物时子包路径不被触碰 | A-WX3 | ✓（见 Actual） |
| P-WX05 | 行为 0 | parser crates `git diff` 零语义；CI vitest 全绿（跨平台证据）；`fe/packages` 零 diff | A-WX4 | ✓（见 Actual） |
| P-WX06 | 发布面 | `npm publish --dry-run`（主包 files/结构）；`napi prepublish` 子包产物正确 | A-WX5 | ✓（见 Actual） |
| P-WX07 | 消融 | ①CI 拔构建步骤（本地模拟 `.node` 缺失）→ 编译测试全炸（已证模式）→ 恢复；②拔双态本地探测 → 单态子包路径断言失败 | A-WX5 | ✓（见 Actual） |
...
- **P-WX01 CI 绿（环境阻塞，声明）**：fork `foolishchow/dimina` Actions 未启用（零 runs + dispatch 403 实证）→ push 不触发；fe-tests.yml 的 Rust 构建/typecheck/test 步骤正确性由**本地等价证据**背书（darwin 全链 584/584 + build script 三态经 napi CLI 机制保证）；真实 CI 绿以 didi 侧回流 PR 运行为准（sidecar 回流时顺带确认）。
...
| P-WX01 | **Uncovered（声明）** | fork Actions 未启用（blocked-on-env）；以 didi 侧回流 PR 运行为准——见下「环境级发现」 |
...
| P-WX03..07 | **pass（P1，见下）** | `7e43f175` 交付：子包结构 / 双态解析 / crate 零语义 diff / dry-run 连通 / 消融 |
