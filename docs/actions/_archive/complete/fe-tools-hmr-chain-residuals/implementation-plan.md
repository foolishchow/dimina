...
- [x] 确认 H2 Phase 2 阶段函数 + registry 实现就位（`logicLoader` / `viewLoadModule` / `styleLoad`/`styleCompile`/`styleEmit` / `LoaderRegistryImpl`）
- [x] 确认 F-HR-1..3 + ③a/b/c + R3 tracker 状态 open（draft 已入档）
- [x] **实施期评项预研（D-HR-1 §2 待评）**：Loader 接口 vs storeInfo graph reconcile 衔接点——load 写图 vs storeInfo 写图唯一权威（PS2 约束）。选项 B 的 load 接线在 storeInfo 现有 graph 写入点内；衔接形状（Loader.dependencies → graph edge 写入点 vs storeInfo reconcile）须在 Step 1 前定型。**⚠️ one-shot 等价性为保证条件**——one-shot build() 也走 orchestrator（`index.ts:9` → orchestrator:189 `_store.load` 产 graph），Step 1 load 接线对 one-shot 同样生效；Loader 写图必须 == storeInfo.load graph 产出（one-shot diff=0 的核心保证）
- [x] **view/style Loader 形状适配（D-HR-1 §2 待评②）**：logicLoader 是整段包装，view/style 是逐模块函数——两种形状共存于一 registry 的接口一致性方案定型
- [x] **StageChannelContext 字段类型来源（R-HR-4 预研）**：`storeInfo` 字段类型来源未定——types.ts 现 import `emit.ts`/`dependency-graph.ts`（type-only，合规）；`storeInfo` 若需 `env.ts` 层类型 → 破“Packer 形状纪律：types.ts 不从 env.ts import”。须定型：从 `env.ts` 提取 `StoreInfo` 类型到 model/shared，或用 opaque/局部类型避免跨层 import

## Step 1 — D-HR-1 registry 接线（locked B）⚠️ partial（(a) done；view/style Loader 形状适配 = Non-scope 接口演进 → (b)/(c) deferred 后续门）
...
## Step 2 — D-HR-2 L_HMR flag-gated（locked b）✅ done
...
## Step 3 — R-HR-4 ctx 类型收敛 + R-HR-5 下沉（小修，独立）✅ done（3a R-HR-5 + 3b R-HR-4）
...
## Step 4 — D-HR-3 selective 链路级测试（locked 边界级）✅ done
...
## Step 5 — 验证（P-HR1..7）✅ done

- [x] P-HR1 `grep -rn "loaderRegistry\.\(get\|kinds\)" src/`（非零且非测试）+ kinds/get 单测断言（logic/view/style）
- [x] P-HR2 L_HMR flag 两态单测：默认关 payload == baseline（dev-reload.spec.js 既有 18 tests + H4 L_HMR 4 tests，复用/扩展）；开 + 增量（stages>0，非限定单 kind——dev-reload.ts:62） → L_HMR + changedStages + affectedPages
- [x] P-HR3 view-selective-stages.spec.js pass（触发/子集/字节三断言）
- [x] P-HR4 `grep -n "ctx as {" src/compiler/pipeline/stage-channel.ts src/packer/orchestrator.ts` ctx 字段全部经 typed 边界（grep ctx as { 在 ctx 字段集 = 0；result/task 局部窄化不计）
- [x] P-HR5 `grep -n "pipeline/" src/model/invalidation.ts` = 0
- [x] P-HR6 行为 0 三件套：`tsc --noEmit` 0 + vitest 全绿 + 6 项目 one-shot `diff -r` = 0
- [x] P-HR7 tracker 状态更新：close 时 F-HR-1/3 fixed + R3 fixed + ③a fixed（③b/③c open）+ F-HR-2 fixed（通道补齐）+ 默认 true deferred（runtime 就绪）；证据链接

## Step 6 — close ✅ done（complete-with-residual）

- [x] architecture-notes 回流（D-HR-1 衔接形状 + D-HR-2 激活条件 + R-HR-4 typed 边界 + R-HR-5 下沉）
- [x] STATUS/TODO/archive 同步；validator 0/0
- [x] residuals tracker：F-HR-1..3 fixed；R3 fixed；③a fixed（③b/③c open）；F-HR-2 **fixed（通道补齐）+ 默认 true deferred（runtime 就绪）**
