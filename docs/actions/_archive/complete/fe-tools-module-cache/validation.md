...
| P-MC05 | 颗粒度 | 两个 entry 引用同一 module，module 计算计数为 1 | **pass（MC3，复用 hotpaths）** — compiler-hotpaths.spec 已验证共享组件只处理一次（cheerioLoad 3 次 = 页面 + 共享组件）；module-cache.spec 验证 style 同内容命中（postcss process 单次） |
