...
| 2a | `style/emit.ts`：加 `import postcss`；迁 `loadCssnano()` + `cssnanoLoader`；`emitStyle` 加 cssnano canonical path（D-CN-1/3；`annotation: false` + `let map` 独立变量） | **done** |
| 2b | `style/parse-walk.ts`：删 loader 定义；import 改 `from './emit.ts'`；cssnano 调用加 `isDiffVerifyMode()` gate（D-CN-2） | **done** |
| 3 | P-CN* / A-CN*；回流 architecture-notes；close 另授 | **done** |
