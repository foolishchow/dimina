...
| A-E5 | R-E5 | 消融有效；契约/output 回流 architecture-notes | 消融由行为 0 对拍（current=emit vs baseline=手写 diff=0）+ grep 锚定共同覆盖；契约/output 已回流 architecture-notes §Emit 层（docs/fe-tools/architecture-notes.md） | ✅ pass |
