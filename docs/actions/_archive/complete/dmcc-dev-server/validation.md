...
- A-001~A-012 全部 passed（A-010 为 SHOULD 支撑项，已通过：P-007 tarball 裸项目服务 sdk 资产）。
- dev server 契约定稿结论回写 RFC（§4.1/§4.2 补充为 §4.5：ws 协议、reloadLevel 合成、快照语义、宿主页语义；§5 A2 行标注完成；修订 v1.3）。
- 覆盖说明：A-004 容器级 relaunch 视觉以 ws L1 载荷 + dev-host.js 代码路径审查 + 浏览器冒烟记录覆盖；真实容器内 relaunch 完整视觉验证为已知残余（A-004 口径允许冒烟记录），A3 宿主页增强时补足。无未记录的未覆盖区域。
