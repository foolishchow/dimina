...
| ID | 验证项 | 命令/方法 | 状态 |
| --- | --- | --- | --- |
| P-FC1 | 6 collaborator 抽取 | `ls packer/collaborator/`（或对应域子目录）7 文件 + git diff -M rename + 逻辑体搬迁 | pending |
| P-FC2 | facade 契约 | orchestrator `implements PackerOrchestrator` + 签名 `(ctx, state, options) → EmitEntry[]` + createPackerOrchestrator 仅 `{ orchestrate }` | pending |
| P-FC3 | OrchestrateRequest 收敛 | 入口 CompileRequest/WatchRequest + 3 调用方 + 测试 mock 同步 | pending |
...
| P-FC6 | ALS 保留 | `grep 'getWorkPath\|getPages\|getAppConfigInfo\|isMiniGame' packer/collaborator/` 非 0 + collaborator 业务逻辑完整 | pending |
| P-FC7 | Non-scope 边界守 | renderer 副作用注册原样（orchestrator.ts:62-70 grep webviewRenderer）+ aspect 穿线保留 + L/C/E NOT wired | pending |
