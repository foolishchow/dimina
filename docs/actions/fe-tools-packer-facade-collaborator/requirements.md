...
packer 架构 retrospect（[F-PA-1..6](../../fe-tools/2026-10-09-packer-facade-aspect-retrospect.md)）发现 orchestrator 是 god object：`_orchestrate` 同时干 6 类业务活（store.load / compileConfig / npmBuilder / deriveLogicBuckets+emitEngine / materialize / publishToDist）+ 公开 4 registry + `OrchestrateRequest` 20+ 字段透传。北星契约 `PackerOrchestrator.orchestrate(ctx, state, options) → EmitEntry[]`（`types.ts:386-393`）**根本没落地**——实际签名 `_orchestrate(request: OrchestrateRequest) → Record<string, unknown>`，签名/入参/返回全不符。

目录收敛（fe-tools-packer-directory-convergence complete）已把 packer/ 9 子目录就位，给本 Action 干净素材。

## R-FC-1 — collaborator 抽取（F-PA-1 落地）

orchestrator `_orchestrate` 6 业务块下沉到**拥有逻辑的 collaborator**（每 collaborator 拥有一类业务活的完整逻辑，非包壳）：
...
- orchestrator `implements PackerOrchestrator`（消解 orchestrator.ts:13 自承"不写 implements"张力）
...
`OrchestrateRequest` 20+ 字段（targetPath/workPath/useAppIdDir/state/store/lifecycle/fileTypes/affectedEntries/stages/seedPath/prepareConfig/prepareNpm/skipMaterialize/invalidatedModules/incremental/configChanged/parallel/compileOptions...）→ 收敛为语义入参：
...
- **不动 renderer 注入点**（F-PA-3，A 切法）——orchestrator.ts:62-70 webviewRenderer 副作用注册 + renderers.ts 索引签名保留原样（另 Action）
