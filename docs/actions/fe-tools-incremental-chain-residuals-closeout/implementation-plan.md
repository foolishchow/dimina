...
| G5 归档 acceptance/validation + architecture-notes | 回写一行（R1 接线后"watch-runner 创建实例"叙事与代码一致） | R-IRC-8 | pending |
...
| 新增 R1 接线回归测 | 经 `createBuildWatcher`（或 watch-runner build path）触发 initial build + rebuild → 断言 `sessionState.viewCache` 非空 + size>0 + 含 page bundle（D-IRC-5，不手建 Map） | R1 | pending |

## Step 4 — 验证（P-IRC-1..N）

- [ ] P-IRC-1 代码审阅（watch-runner 接线 + one-shot 不动 + 注释 + doc 对齐）
- [ ] P-IRC-2 R1 接线回归测 pass（真实 watcher 路径，不手建 Map）
...
- [ ] P-IRC-6 watch 字节恒等：真实 watcher 路径 cache 启用 → build1 vs rebuild 产物 view/style 字节一致
...
- [ ] G5 归档 acceptance/validation + architecture-notes 回写一行
