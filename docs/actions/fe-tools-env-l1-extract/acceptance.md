...
| A-EL1-4 | R-EL1-4 | resolveAppAlias 迁出 | resolveAppAlias 迁 env-compute 收 appInfo 参数 `(src, appInfo)`；env.ts 保留 wrapper（选项 A 锁定——读 ALS appInfo + 调 env-compute，import alias）；parse-walk.ts:312 调用与 import 均不变；tsc 0 | pending |
