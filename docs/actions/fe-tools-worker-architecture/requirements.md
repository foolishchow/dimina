...
## R-WA1（MUST）演进路径定稿（归属与验收入口）

交付四阶段演进路径（结果边界 → 协变 → 模型层 → 可选常驻 service），每阶段标注：
- 归属 Action（阶段 1/2 → build-model；阶段 3 → module-cache；阶段 4 → 未来）
- 交付边界（改什么/不改什么）
- 验收入口（字节等价/行为断言由归属实施 Action 执行；本 Action 只验证映射完整、无阶段无主/双主）

**本 Action 不运行阶段验收**（决策 Action 定位）——只保证演进路径的归属与验收入口声明完整。
