...
| --- | --- | --- | --- | --- |
| P-BP01 | 全量回归 | `vitest run --no-file-parallelism`（tools/bundler） | A-BP01 / A-BP02 | **pass** — 485 tests / 73 suites 全绿（新增 `__tests__/project-store.spec.js` 4 用例，见 P-BP04） |
...
| P-BP04 | Store 注入 / M-A 保持 | `__tests__/project-store.spec.js`：load 恰好 1 次 / getDependencyGraph 被调用 / captured graph ≡ build 结果 / 无 store 临时路径 | A-BP04 | **pass（审查 Round 2 补）** — 4/4 绿；消融证实测试锁定「注入 store 被忽略」缺陷（见消融记录 §A） |
