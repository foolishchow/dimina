...
| P-PS04 | watch 刀 A / **PS2** | watch-runner.spec（6 用例，+2 PS2）；session.watch 注入 state.store；无 store 直调临时 store（W2）；**闭包镜像已删（W3→仅 Store）** | A-PS04 | **pass（PS1+PS2）** — watch/dev 全绿；注入 store 同一引用传 build；无 store 临时 create 复用；闭包 `dependencyGraph` 变量与 `new DependencyGraph` 已从 watch-runner 删除 |
