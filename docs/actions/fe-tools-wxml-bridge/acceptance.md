...
| A-WB2 | R-WB2 | view `inMap` 由真 span 构建；include/import 跨文件归位（行级正确 + 列级可用） | P-WB04 / P-WB02 | pending |
| A-WB3 | R-WB3 | code 严格 diff=0；**行级不变量**（无 include/import 页 = 今日 1:1）；更准断言（抽查集 = 真 {file,line}） | P-WB02 / P-WB04 | pending |
