...
- **map 允许变化**（修复即变）且**必须更准**：
  - **行级语义正确（不变量）**：无 include/import 页面的映射行目标 = 真实 {file,line}（今日 1:1 猜射若恰好正确则一致；否则以真 span 为准——1:1 只是对照基线，非硬断言目标）；
  - **差异观测（F14）**：与今日映射的差异作为观测记录（简单页行保持率 checkpoint），不构成验收硬断言；
  - **实证决策门（W2 Step 0）**：对 base 无 include 页跑一次行保持率探针，据实落定不变量形态（"等于今日"仅当行结构保持成立，否则用语义正确形态）；
