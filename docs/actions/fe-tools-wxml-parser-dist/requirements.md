...
Status: **冻结（2026-09-15）** — D-WX-1..8 已拍板；随 Action `ready`。
...
- **build script 跨平台化**：现状 `pnpm build` 硬编码 `cp …libdimina_wxml_parser_napi.dylib`（darwin 专属）——linux 产物是 `.so`、windows `.dll`；P0 必须三态正确（迁 `@napi-rs/cli napi build` 或等价）。
- CI 全绿（以 push 后 GitHub Actions 运行为准；本地等价证据先行）。

## R-WX1（MUST）P0：`.node` 退 git（D-WX-7）

- `git rm --cached fe/tools/wxml-parser-napi/index.node`；`.gitignore` 生效。
...
## R-WX3（MUST）P1：双态解析（D-WX-3）

- `index.js` 加载序：**本地 workspace 现场 `./index.node`（dev `pnpm build` 产出）优先**；不存在 → 平台检测走子包；皆无 → 现行 `[wxml]` 延迟抛错。
...
- 消融：CI 拔构建步骤 → 红（回归价值锚定）；拔本地 `.node`（子包亦无）→ 延迟抛错文案正确。
