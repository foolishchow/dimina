...
| 文件 | 生产者 | 内容 | git |
| --- | --- | --- | --- |
| `index.<platform>.node` | `napi build` | 原生产物（`.dylib`/`.so`/`.dll` → `.node` 由 CLI 处理） | **不追踪**（D-WX-7） |
| `binding.js` | `napi build`（`--js`，ESM） | CLI 平台检测 + 子包选择（P1 发布态正用它） | 追踪（生成物入库，见 Residual） |
| `index.js` | **手写保留** | 双态加载（本地 `index.<platform>.node` 优先 → `require('./binding.js')` → 延迟抛错）+ `parseWxmlSpanView` 包装 | 追踪 |
...
```js
// 加载序（双态）：
// ① 本地 dev：./index.<platform>.node（pnpm build --platform 产出，git 不追踪）
//    存在 → 直接用之（workspace dev 不需 npm install 子包）
// ② 发布态：require('./binding.js')（CLI 生成：平台检测 → optionalDependencies 子包）
// ③ 皆无 → 延迟抛错（现行文案，含 pnpm build 指引）
```

（**R1-F1 修正**：文件名带平台后缀，与 `napi build --platform` 产物对齐。）`parseWxmlSpanView` / 错误格式化 / `_nativeForTest` 面零改动（全在手写 `index.js`）。双态选择逻辑以可测钩暴露（`_resolveNative()`，**R1-F6 落点：bundler `__tests__/wxml-parser-loader.spec.js`**，供 P-WX04 单测）。
