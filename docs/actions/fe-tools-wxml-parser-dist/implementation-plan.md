...
| Step | 文件 | 动作 |
| --- | --- | --- |
| 1 | `wxml-parser-napi/package.json` | devDep `@napi-rs/cli`；`build` → `napi build --platform --release --js binding.js --format esm --no-dts --manifest-path ../crates/Cargo.toml`（**D-WX-9**：CLI glue 写 `binding.js` 不覆盖手写 `index.js`）；`napi` 配置块（host target 先行）；pnpm install |
