...
| --- | --- | --- | --- | --- |
| A-WX0 | R-WX0 | build 三态正确（napi CLI `--platform`）；**P0 最小加载** `index.<platform>.node` 可用；CI 构建步骤 + cache 在档；**Actions 全绿**（run 锚定） | P-WX00/P-WX01/P-WX02 | pending |
| A-WX1 | R-WX1 | `*.node` 不在 git；`.gitignore` 覆盖平台后缀产物；dev 前置成文（Rust + pnpm build）且本地流全绿 | P-WX02 | pending |
| A-WX2 | R-WX2 | 五平台子包 + 主包 `optionalDependencies` + 解除 private；发布工作流在档 | P-WX03 | pending |
| A-WX3 | R-WX3 | 完整双态（本地 `index.<platform>.node` → `binding.js` 子包 → 抛错）实证；bundler 零改动 | P-WX04 | pending |
