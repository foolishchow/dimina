...
3. **Output 生命周期（D-OL1..4，F1）**：
   - config-collector 设 `sctx.output = new MemOutput()`（dev 分支，替代 sctx.buildModel = new BuildModel()）
   ...
   - session L244 `state.output = buildResult.output`（替代 state.buildModel）
   - session L255 `build:end` listener → `state.output = result?.output`（rebuild 替换，复刻 state.buildModel rebuild）
4. `session/index.ts` dev 模式（config-collector 或 session）构造 MemOutput 注入 sctx + dev server
...
2. config-collector：one-shot + previewAdapter + watch standalone（F4）→ `sctx.output = new DiskOutput(final)`（替代 MemOutput）；dev（无 previewAdapter）→ MemOutput 保留
3. `bin/compile.ts` / `index.ts` build facade：one-shot final=TARGET_PATH（config-collector 已按 mode 选 DiskOutput）
4. `session/index.ts` previewAdapter-dev 分支：config-collector 构造 DiskOutput（现状 skipMaterialize=false 路径）
