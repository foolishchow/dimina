...
测试覆盖真实生产链（至少 stage-channel → worker → compileML 边界，层级由 D-HR-3 锁）：(a) selective 分支真实触发；(b) pageBundles 只含 dirty 子集 + orderList 全量；(c) selective pageBundles（dirty 子集 + cached clean 经 orderList 组装）与全量重编 pageBundles 字节恒等。
