...
| --- | --- | --- | --- | --- |
| A-HR1 | R-HR-1 registry 管线接线 | `loaderRegistry.get/kinds` 有生产调用点（grep 非零）；view/style Loader 已注册；compile/emit registry 非空（阶段函数注册） | grep + registry 内容 dump + 单测 | pending |
...
| A-HR4 | R-HR-4 R3 ctx 类型收敛 | `stage-channel.ts` / `orchestrator.ts` 结构断言清零（grep `as { viewCache` = 0）；单一 typed 边界声明在档 | grep + tsc | pending |
| A-HR5 | R-HR-5 COMPILE_STAGE_ORDER 下沉 | `model/invalidation.ts` 无 `compiler/pipeline` import（grep = 0） | grep + vitest | pending |
