...
| P-HR2 | L_HMR flag 两态 | 单测：默认关 payload == baseline（L1/L2/L3 不变）；开 + 单 kind 增量 → L_HMR + changedStages + affectedPages | pending |
| P-HR3 | selective 链路级 | 新 spec：两轮 build（priming + invalidate）经 stage-channel 边界；断言 selective/dirty/orderList/字节恒等 | pending |
| P-HR4 | ctx 断言清零 | `grep -n "as { viewCache\|as { buildModel" src/compiler/pipeline/stage-channel.ts src/packer/orchestrator.ts` = 0 | pending |
...
| P-HR7 | tracker 同步 | residuals tracker 修订 diff review | pending |
...
- registry dispatch 全替换（D-HR-1 选项 A）——本 Action 选 B，A 留后续门
