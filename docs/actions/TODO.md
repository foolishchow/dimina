...
- [`fe-tools-hmr-registry-materialize`](fe-tools-hmr-registry-materialize/README.md)（**`draft`**）— HMR-compiler H2 子门：emptyRegistry → 实体化（Loader/Compiler/Emitter 替代 compile-target）。D-REG-1 渐进 + D-REG-2/3 load/stage 归属。design.draft 实证待做。
- [`fe-tools-hmr-per-module-cache`](fe-tools-hmr-per-module-cache/README.md)（**`draft`**）— HMR-compiler H3 子门：G5 per-page-bundle → per-module。D-PMC-1 stored order metadata（避开 P-G506）+ per-module invalidation。design.draft 实证待做。
- [`fe-tools-hmr-push`](fe-tools-hmr-push/README.md)（**`draft`**）— HMR-compiler H4 子门：dev-reload L_HMR + 增量 payload + materialize 增量化。runtime fallback L1。design.draft 实证待做。
