...
- [`fe-tools-graph-persist`](fe-tools-graph-persist/README.md)（**`draft`**）— storeInfo state 路径走 reconcile（非 build），保留旧图 source-level edges。增量前置 G1。
