...
| 其后 | 伞级 complete 归档；M0 emit W1 另门可独立先行；后继伞 [`fe-tools-module-convergence`](_archive/complete/fe-tools-module-convergence/README.md) **`complete`**（MC0 graph 正确 + MC3a deriveFromGraph 全 complete；D-MC-0 **A=code 不上图**） |
