...
- [`fe-tools-view-style-cache-skip`](fe-tools-view-style-cache-skip/README.md)（**`ready`**）— 增量前置 G5（incremental-unify 重激活）：A-IU-3 剩余（PackerSessionState 字段 + orchestrator plumbing + worker input 快照）+ A-IU-4 cache-hit skip（view/style 跳 compile re-emit cached）。闭合 incremental-unify。设计输入 = D-IU-2/3/4/5 + G4 D-G4-1..9。design gate RG5-1 ◑ 近解（F6 allCached+③+ONE emitEntry；residual=验证题）/RG5-2 ✅/RG5-3 ✅/RG5-4 ✅。18 轮 review 12 findings 全清（含 F12 bare Map）。行为 0（one-shot diff=0；watch cache-hit=效率提升）。
