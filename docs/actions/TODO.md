...
- [`fe-tools-style-minify-path-unify`](fe-tools-style-minify-path-unify/README.md)（**`ready`**）— 统一 style minify 到单一字节恒等路径（per-module + 保留模块间 `\n`）→ 下线 `DIMINA_COMPILER_DIFF_VERIFY` dual-path。**反转 D-SM-4/D-CN-4 Non-scope**（字节一致现为要求）。闭合验证缺口（6 轮 behavior-0 验 parse-walk 路径，production 跑 emit 路径——字节不同，probe 实证）。**D-SMPU-2 locked A**（revert，minify 留 parse-walk，最小改动 + 字节恒等已证 + 接受放弃 D-SM-2/D-CN-1/D-CN-3 迁移）。前置：style-minify-gate + style-cssnano-gate（complete）。
