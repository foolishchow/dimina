...
- [`fe-tools-hmr-chain-residuals`](fe-tools-hmr-chain-residuals/README.md)（**`ready`**）— 伞 close 后复盘浮出"三档证据分级"（代码/接线/证据）：F-HR-1 loaderRegistry 注册零消费、F-HR-2 enableHmr 无生产设值点（与 D-PUSH-2 locked 不符）、F-HR-3 selective 仅 compileML 直调无链路证据；+ R3（条件过期）+ ③ 下沉。design gates：D-HR-1 接线程度（荐 B 首消费点渐进）、D-HR-2 激活策略（荐 b flag-gated 默认关）、D-HR-3 测试层级（荐 stage-channel 边界级）。
