...
- [`fe-tools-hmr-emit-derive`](fe-tools-hmr-emit-derive/README.md)（**`ready`**）— HMR-compiler H1 子门：orchestrator Logic emit task 改调 deriveFromGraph（非 emitBuckets）。entry 映射（per-entry→per-bucket）挑战 + D-ED-2 locked B（一次性）。logic-only。design.draft 实证 pass（B2+E locked）。
