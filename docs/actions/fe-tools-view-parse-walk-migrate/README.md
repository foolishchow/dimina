...
- 设计门：[design.draft.md](design.draft.md)（**D-VPM-1..N 待 lock**——view parse-walk ctx 迁移）
...
- D-VPM-3：view/parse-walk 17 处 getter 改 ctx 读（ctx 读 5 getter + 保留 ALS 3 getter）
...
1. **getViewScriptExts/getViewScriptTags ctx 读**：ctx.fileTypes.viewScriptExts/viewScriptTags 与 ALS 等价确认
2. **viewParseWalk 内部函数透传**：compileViewTree 等内部函数 17 处 getter 分布——ctx 须透传内部函数 or 顶部建 local
3. **view/index.ts getWorkPath L72 + successPayload**：viewCompile 建 ctx 后 L72 改 ctx + successPayload 保留 ALS（getDependencyGraph A5 统一迁）
