...
| P-IT02 | 增量行为 0 | 全量路径：4 组 diff=0（nomap/min-nomap/sm/sm-min）证明改后 derive 产物不变；增量路径：`compile-cache.spec.js`（`affectedEntries`/`stages` 选项袋 12 处）+ `build-stages.spec.js`（经 `build()` 全链路测 `affectedEntries`/`stages`）+ `compile-target.spec.js`（直接测 `deriveStagePlan` 含过滤语义）既有测例全绿 | A-IT1 | pending |
