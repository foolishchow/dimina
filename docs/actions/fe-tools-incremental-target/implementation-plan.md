...
| 2 | `compile-target.ts` | 搬入 `filterPagesByEntries` 函数（从 build-pipeline.ts）；签名收紧为 `(pages: PagesInfo, affectedEntries: string[] \| undefined): PagesInfo`（当前返回 `{ mainPages: unknown[]; subPages: Record<string, unknown> }` → `PagesInfo`；`PagesInfo` index signature `[key: string]: unknown` 使函数体不改） | 搬迁不改逻辑 ✓ |
