...
| H4 per-module HMR push | **`complete`**（子门 [`fe-tools-hmr-push`](../_archive/complete/fe-tools-hmr-push/README.md)） | M | runtime 协议依赖；D-PUSH-1/2/3 locked（L_HMR payload + runtime-side downgrade fallback L1 + materialize 增量 + publishToDist 重构 F-H4-2）；非阻塞伞 close |
