...
| --- | --- | --- | --- | --- |
| A-HMR1 | R-HMR-1 deriveFromGraph 接线 | production emit 路径走 deriveFromGraph（非 emitBuckets）；只 emit 受影响 modules | 子门 H1 acceptance + diff=0 | pending |
| A-HMR2 | R-HMR-2 registry 实体化 | orchestrator emptyRegistry → real Loader/Compiler/Emitter；compile-target → registry 路径 | 子门 H2 acceptance + diff=0 | pending |
| A-HMR3 | R-HMR-3 per-module view/style cache | view/style cache 从 per-page-bundle → per-module；单组件 recompile 可行 | 子门 H3 acceptance + watch 字节恒等 | pending |
| A-HMR4 | R-HMR-4 per-module HMR push | dev-reload HMR level + dev-server 增量 payload；dev server 消费增量（非全量 reload） | 子门 H4 acceptance + dev server 验 | pending |
