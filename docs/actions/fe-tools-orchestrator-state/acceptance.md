...
- [ ] watch rebuild 时 `state.graph` 已有上一次 build 的数据
- [ ] storeInfo 调 `graph.reconcile()`（不调 `restoreFromSnapshot`）
- [ ] graph 跨 rebuild 持久（同一 PackerGraph 实例）
- [ ] worker delta（source-level edges）跨 build 保留
...
- [ ] watch-runner 传 `dependencyGraph: state.graph` 给 `createWatchBuildPlan`
- [ ] `state.graph` 签名兼容 `createWatchBuildPlan` 的 `dependencyGraph` 参数
- [ ] `hasFile()` 返回真实结果（非空图）→ 增量路径可生效

## A-OS-6 — 行为 0（R-OS-6）

- [ ] 单次 build：7 examples 全 diff=0
- [ ] watch rebuild：产物与 full rebuild 一致
- [ ] vitest 全绿（608+，含 watch-runner.spec.js）
