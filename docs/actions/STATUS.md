...
| `fe-tools-ts-migration` | `ready` | [README](fe-tools-ts-migration/README.md) | **bundler/src 全仓 72 .js→.ts**（src/compiler 41 + shared/model/session/watch/dev/bin/根 31）；import 后缀显式 .ts（D-TM-1 拍定方案 A，POC 验证）；行为 0（diff=0 + 584/584）；JSDoc→TS type 类型完善（不用 any，D-TM-4 选项 B）；分阶段 core+shared→worker-runtime→pipeline+model+session+watch→view+logic+style→bin+dev+根（D-TM-3 拍定）。R1-R4 收敛，1134 类型错误分阶段类型化。 | 2026-09-16 |
