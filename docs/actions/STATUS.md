...
| `fe-tools-emit-w1-parameterize` | `draft` | [README](fe-tools-emit-w1-parameterize/README.md) | 焊点 1（emit.ts）纯化：EmitEntryParams 加 workPath 参数替代 getWorkPath() 全局函数调用；emit.ts 0 env.ts 依赖。S 级行为 0 重构。**讨论后判断价值不足**（1/16 耦合点，无用户价值），由 `fe-tools-packer-core-shape` 的形状定义取代。 | 2026-09-21 |
| `fe-tools-packer-core-shape` | `draft` | [README](fe-tools-packer-core-shape/README.md) | Packer core 5 组件形状定义（非抽取）：PackerContext / PackerModule / Packer API / 模块生命周期 / Orchestrator（编排器）。产出 TypeScript interface 声明 + 边界文档。行为 0。北星契约——watch/HMR/deriveFromGraph 都挂在它上面。 | 2026-09-21 |
