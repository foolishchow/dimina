...
| `fe-tools-graph-bootstrap` | `draft` | [README](fe-tools-graph-bootstrap/README.md) | storeInfo config fixpoint 迁入 Graph（D-PCS-2, D-PCS-4）。PackerGraph 实现 Graph interface，自包含读 app.json→递归组件→建图。storeInfo 瘦身为 PackerContext only。ALS 兼容。行为 0。前置 fe-tools-packer-core-shape。 | 2026-09-21 |
| `fe-tools-incremental-unify` | `draft` | [README](fe-tools-incremental-unify/README.md) | view/style 模块级增量接入。computeInvalidatedModules 泛化全 kind（不 only logic）；view/style ModuleResultCache 接入（worker 返回 result + cache hit 跳过）。行为 0。前置 fe-tools-packer-core-shape + M1+M2。 | 2026-09-21 |
| `fe-tools-als-store` | `draft` | [README](fe-tools-als-store/README.md) | 通用 ALS 工具类 AsyncContextStore<T>（run/get/tryGet + globalThis 兜底）。统一 worker-runtime abilityContext + env.ts compilerContextStorage 两处 AsyncLocalStorage。abilityContext 从 unknown→有类型。行为 0。Packer 落地地基。 | 2026-09-21 |
