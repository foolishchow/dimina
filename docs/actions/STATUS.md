...
| `fe-tools-incremental-target` | `ready` | [README](fe-tools-incremental-target/README.md) | E7 增量形态回灌：deriveStagePlan 显式消费 affectedEntries（删 pipeline filterPagesByEntries 私算）+ COMPILE_STAGE_ORDER 单源（S4）；方案 A（字段名不变）；D-IT-1..4 全拍板；source audit 实锚。待授权 in_progress。 | 2026-09-19 |
