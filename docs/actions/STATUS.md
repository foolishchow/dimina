...
| `fe-tools-als-store` | `ready` | [README](fe-tools-als-store/README.md) | 通用 ALS 工具类 AsyncContextStore<T>（run/get/tryGet + globalThis 兜底）。统一 worker-runtime abilityContext + env.ts compilerContextStorage 两处 AsyncLocalStorage。abilityContext 从 unknown→有类型。行为 0。Packer 落地地基。 | 2026-09-22 |
