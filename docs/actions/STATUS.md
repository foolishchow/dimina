...
| `fe-tools-view-parse-walk-cleanup` | `draft` | [README](fe-tools-view-parse-walk-cleanup/README.md) | view/parse-walk.ts 内部质量重构（行为 0）：拆 compileResCache 三用途→3 Map+CacheEntry type；提取 mergeWxsModules 去重；拆 compileModule/processWxsContent/insertWxsToRenderResult 长函数。非目标：跨车道 AST 共享、缓存泄漏修复、二次编译消除。 | 2026-09-21 |
