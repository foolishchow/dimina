...
| `fe-tools-packer-core-shape` | `draft` | [README](fe-tools-packer-core-shape/README.md) | Packer core 5 组件形状定义（非抽取）：PackerContext / LoadedModule+CompiledModule / Packer API / 模块生命周期 / Orchestrator。load→compile→emit 3 环节。产出 TypeScript interface 声明 + 边界文档。行为 0。北星契约。前置 fe-tools-packer-lifecycle-audit。 | 2026-09-21 |
