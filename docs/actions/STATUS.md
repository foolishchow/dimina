...
| `fe-tools-build-model` | `draft` | [README](fe-tools-build-model/README.md) | 内部统一第一步（**主线程层**）：BuildModel（Entry 产物持有）+ `(mtime,size)` 预筛指纹 + scan/closure 失效传播 + materialize 统一物化；**对拍为 M2 MUST 子项**。不碰 IR/transform/env/FileModule。M1（=阶段1）→ M2（=阶段2）→ M3 可选（仅 cache 迁移）。 | 2026-09-10 |
| `fe-tools-module-cache` | `draft` | [README](fe-tools-module-cache/README.md) | **worker 内层（与 build-model 并行）**：ModuleCache/FileModule 颗粒化 + compileResCache 内容寻址化 + 失败缓存 + key 维度（minify/esTarget/fileTypes/renderer）+ scriptRes 副作用等价验收；不拆 view 组合（TS-2）、不持有到主线程。MC1..3（=阶段3）。 | 2026-09-10 |
| `fe-tools-worker-architecture` | `ready` | [README](fe-tools-worker-architecture/README.md) | **决策 Action（ready=冻结，不实施）**：四阶段演进路径 + D-WA-1..6（含依据列/修订机制）+ WorkerTask/Result 协议（v1/v2 两代演进 + stats 消费契约 §4.1 + 消息时序防护 §4.2）。经 10 轮审查 + ready 评审通过。 | 2026-09-10 |
