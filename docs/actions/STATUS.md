...
| `fe-tools-build-model` | `ready` | [README](fe-tools-build-model/README.md) | 内部统一第一步（**主线程层**）：BuildModel（Entry 产物持有）+ `(mtime,size)` 预筛指纹 + scan/closure 失效传播 + materialize 统一物化；**对拍为 M2 MUST 子项**。D-BM-1..7 + D-P1..4 冻结。M1（=阶段1）→ M2（=阶段2）→ M3 可选。 | 2026-09-10 |
