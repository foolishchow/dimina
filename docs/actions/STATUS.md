...
| `fe-tools-bundler-emit-memfs` | `draft` | [README](fe-tools-bundler-emit-memfs/README.md) | 刀 1 后产物面统一：dev memfs（产物不落盘）+ 漏网点收口（materialize/app-config/publish）+ 目录归置；cache「家」拍板分叉点；emit 层零改动；拍板链未走完。 | 2026-09-16 |
