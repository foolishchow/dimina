...
| `fe-tools-parse-walk-extract` | `ready` | [README](fe-tools-parse-walk-extract/README.md) | view/style parse-walk 真抽出（从 re-export 壳升级为真搬代码）；3 条 TD fallback → 正式决策（collectAllWxsModules 保留 / page sourcemap 条件 / minifyCss per-module）。承接 emit-transform-split 的 3 条已知 debt。行为 0。 | 2026-09-21 |
