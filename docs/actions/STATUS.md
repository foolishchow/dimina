...
| `fe-tools-module-cache` | `in_progress` | [README](fe-tools-module-cache/README.md) | **worker 内层（与 build-model 并行）**：ModuleCache/FileModule 颗粒化 + ComposeCache（组合层）+ compileResCache 内容寻址化 + 失败缓存 + key 维度对齐 contextFingerprint。D-MC-1..4 冻结。MC1（=阶段3）实施中。 | 2026-09-10 |
