...
| `fe-tools-bundler-boundaries` | `ready` | [README](fe-tools-bundler-boundaries/README.md) | Packer/Scheme 职责与目录/文件落点；不改 src。D-BD-1..6 全拍板（4 焊点：emit.ts/logic/env/dependency-graph）；落点表封口全集；Readiness 两轮 review pass；待授权 in_progress。 | 2026-09-18 |
