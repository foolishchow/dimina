...
| `fe-tools-build-pipeline` | `in_progress` | [README](fe-tools-build-pipeline/README.md) | BuildPipeline **BP1 已交付**（`5d29a67b`）：阶段表等价抽取→`src/compiler/build-pipeline.js`；index.js 薄委托；Listr 仍在；行为 0（nomap 94 / sourcemap 185 diff=0）；**待审查闭环**。 | 2026-09-12 |
