...
| `fe-tools-sidecar` | `draft` | [README](fe-tools-sidecar/README.md) | Umbrella 伞级收尾（2026-09-20 书面确认）：TS-0 冻结 + TS-1/TS-2/TS-4 complete + **TS-3 书面 deferred**；终态 B 可检查；packages 不依赖 tools；闭合条件全满足。仍 `draft` 仅因 gap 4（CI，外部约束：fork Actions 禁用 → CI green 推迟到回流 PR）。伞级不再授权大实施；后续候选独立 formalize。 | 2026-09-20 |
