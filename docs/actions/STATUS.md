...
| `fe-tools-packer-lifecycle-audit` | `draft` | [README](fe-tools-packer-lifecycle-audit/README.md) | Packer（= 整个 bundler = session 完整生命周期）全流程梳理：入口/第一build/Worker/watch rebuild/graph生命周期/cache生命周期/5组件映射现状/关键发现F-1..F-6。research only。fe-tools-packer-core-shape 的事实基础。 | 2026-09-21 |
