...
| `fe-tools-build-model` | `draft` | [README](fe-tools-build-model/README.md) | 内部统一第一步：BuildModel（Entry 产物持有）+ 两层指纹（mtime+hash）+ scan/closure 失效传播（替代双 plan 事件推导）+ materialize 统一物化；不碰 IR/transform/env。M1 等价重构 → M2 指纹失效（含 watch 合并事件行为改进）→ M3 可选（cache 迁移+对拍）。TS-2 地基层。 | 2026-09-10 |
| `fe-tools-module-cache` | `draft` | [README](fe-tools-module-cache/README.md) | **build-model 前置地基**：worker 内 ModuleCache/EntryCache 边界 + compileResCache 内容寻址化 + 失败缓存 + key 维度完备（minify/esTarget/fileTypes/renderer）；不拆 view 组合（TS-2）、不持有到主线程（build-model）。MC1 边界寻址 → MC2 key 维度 → MC3 可测性。 | 2026-09-10 |
| `fe-tools-worker-architecture` | `draft` | [README](fe-tools-worker-architecture/README.md) | **决策 Action（ready=冻结，不实施）**：三 worker 演进路径四阶段（结果边界→协变→模型层→常驻可选）+ D-WA-1..6 决策表 + WorkerTask/Result 协议草案；阶段 1/2→build-model、3→module-cache、4→未来。 | 2026-09-10 |
