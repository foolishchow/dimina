...
| `fe-tools-wxml-parser-dist` | `in_progress` | [README](fe-tools-wxml-parser-dist/README.md) | P0 `c2d792de` + P1 `7e43f175`：完整双态（binding.cjs）+ 五平台矩阵 + 子包模板 + 发版流（dry-run 连通）；584/584；**P-WX01 blocked-on-env**（fork Actions 未启用——需网页启用或以 didi 侧为准）。 | 2026-09-15 |
