...
| `fe-tools-ts-migration` | `draft` | [README](fe-tools-ts-migration/README.md) | **bundler/src 全仓 72 .js→.ts**（src/compiler 41 + shared/model/session/watch/dev/bin/根 31）；import 后缀显式 .ts（D-TD-20 tsc rewrite）；行为 0（diff=0 + 584/584）；分阶段 core+shared→worker-runtime→pipeline+model+session+watch→view→logic/style+dev+bin+根。 | 2026-09-16 |
