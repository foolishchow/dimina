...
| `fe-tools-graph-bootstrap` | `in_progress` | [README](fe-tools-graph-bootstrap/README.md) | storeInfo config fixpoint 迁入 Graph（D-PCS-2, D-PCS-4）。PackerGraph 实现 Graph interface，自包含读 app.json→递归组件→建图。storeInfo 瘦身为 PackerContext only。ALS 兼容。行为 0。前置 fe-tools-packer-core-shape。 | 2026-09-22 |
