...
| `fe-tools-parse-walk-extract` | `in_progress` | [README](fe-tools-parse-walk-extract/README.md) | view/style parse-walk 真抽出（从 re-export 壳升级为真搬代码）；3 条 TD fallback → 正式决策（collectAllWxsModules 保留 / page sourcemap 条件 / minifyCss per-module）。承接 emit-transform-split 的 3 条已知 debt。行为 0。 **Step 1+2 实施完成**——style/parse-walk.ts 真抽出（8→450 行），view/parse-walk.ts 真抽出（8→1360 行），tsc 0 错 vitest 608/608 diff=0。 | 2026-09-21 |
