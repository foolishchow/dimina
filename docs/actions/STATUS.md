...
| `fe-tools-sidecar` | `draft` | [README](fe-tools-sidecar/README.md) | Umbrella（draft）；近端 project-store **in_progress**（PS1 已交付）+ build-pipeline **complete**（已归档）；TS-2 deferred；TS-4 成文。 | 2026-09-12 |
| `fe-tools-project-store` | `in_progress` | [README](fe-tools-project-store/README.md) | ProjectStore **PS1 已交付**（`aa6b6508`）：壳 + 刀 A + M-A（ctx.dependencyGraph ≡ store.getDependencyGraph()）+ session/watch 接线；**PS2/PS3 待实施**。 | 2026-09-12 |
