...
| `fe-tools-view-style-cache-skip` | `draft` | [README](fe-tools-view-style-cache-skip/README.md) | 增量前置 G5（incremental-unify 重激活）：A-IU-3 剩余（PackerSessionState viewCache/styleCache 字段 + orchestrator plumbing + stage-channel worker input 快照）+ A-IU-4 cache-hit skip（view/style compileML/compileSS 跳 compile，re-emit cached）。闭合 incremental-unify（A-IU-1..5 全 complete）。设计输入 = incremental-unify D-IU-2/3/4/5 + G4 D-G4-1..9。design gate RG5-1（view cache-hit 递归 emit 语义，倾向 graph 'component' 边）/RG5-2（dependencies:[] 填充）/RG5-3（style per-page ✅）/RG5-4（intra 协同）待 review。行为 0（one-shot 无 invalidated→无 skip→diff=0；watch cache-hit=效率提升）。 | 2026-10-08 |
