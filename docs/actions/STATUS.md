...
| `fe-tools-worker-architecture` | `draft` | [README](fe-tools-worker-architecture/README.md) | **决策 Action（ready=冻结，不实施）**：四阶段演进路径 + D-WA-1..6（含依据列/修订机制）+ WorkerTask/Result 协议（v1/v2 两代演进 + stats 消费契约 §4.1 + 消息时序防护 §4.2）。 | 2026-09-10 |
