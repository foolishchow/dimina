...
| `fe-tools-bundler-emit-memfs` | `draft` | [README](fe-tools-bundler-emit-memfs/README.md) | 阶段 1：worker 无 fs / 干净的 output（方案 C）；output.write→postEntry 纯 postMessage；emitEntry 删 outputEnv；collectOutput 全链路清理；materialize 不动；memfs 另立阶段 2。 | 2026-09-16 |
