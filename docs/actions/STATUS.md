...
| `fe-tools-wxml-refactor` | `ready` | [README](fe-tools-wxml-refactor/README.md) | wxml 三层次打包：W1 函数归位 + W2 **Document 标准形状**（attrs→Attr[]、特殊节点类型化、null/[] 契约语义）+ W3 napi 引擎装配（WXML_PARSER 默认 napi）；D-WR-1..9 全拍板 + 五件套齐；实施待授权。 | 2026-09-15 |
