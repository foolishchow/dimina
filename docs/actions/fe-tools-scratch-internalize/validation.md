...
| V-SI-6 | R-SI-3 | grep `state\.scratch = r\.pathInfo` 在 env-compute storeInfoCtx = 0（去 mkdtemp）+ computeStoreInfo 返回 pathInfo 无 targetPath（grep `pathInfo: localPathInfo` 含 workPath only） | pending |
...
| V-SI-10 | R-SI-5 | **D-SI-5 风险验**：dev 模式（MemOutput）scratch 处置——若 dev 须 scratch 则 MemOutput mkdtemp；若不须则分支。7-diff 含 dev 路径？7-diff 是 one-shot disk——dev 不在 7-diff。须 vitest dev spec 覆盖（dev-server.spec 等） | pending |
