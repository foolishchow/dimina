...
| P-BM04 | 指纹单测 | `(mtime, size)` 未变跳 hash / 内容变化（同 mtime 不同 size）必算 hash / include 聚合稳定 / scan 纯函数性质 | pending（M2） |
| P-BM05 | watch 增量对拍 | 变更单文件 → 仅受影响 Entry 重编译（观察重编译日志/产物 mtime）；合并事件（快速连改）不再全量 | pending（M2） |
| P-BM06 | diff 范围 | 三不动清单（transform 内部 / env.js / 上下文协议段）零改动；worker 协议仅增产物字段 | **pass（M1）** — env.js 零改动 / transform 内部零改动 / 协议仅增 collectOutput + output 字段（git diff 审查，ef1ab730） |
