...
| --- | --- | --- | --- | --- |
| A-BM01 | R-BM1 | worker 产物经 postMessage 回传并被 BuildModel 持有；正常构建路径（collectOutput=true）零 `writeFileSync`（worker 内）；兼容分支（collectOutput=false）保留直接写盘供直连 worker 场景 | 测例（BuildModel 收到产物）+ 源码审查（正常路径写盘零命中） | pending |
