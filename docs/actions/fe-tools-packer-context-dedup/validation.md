...
| --- | --- | --- | --- |
| V-PCD-1 | R-PCD-7 | tsc 0 errors（`node ./node_modules/typescript/bin/tsc --noEmit`） | pending |
| V-PCD-2 | R-PCD-7 | vitest 全绿（88 files 648 tests；flaky solo pass——compile-cli-cache/lifecycle-integration/session-unify） | pending |
| V-PCD-3 | R-PCD-7 | **one-shot 7 项目 build diff=0**（air-battle/base/mpx-demo/subpackages/taro-todo/vant/weui） | pending |
