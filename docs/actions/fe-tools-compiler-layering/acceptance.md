...
| --- | --- | --- | --- | --- |
| A-CL0 | R-CL0 | 目标结构存在：view/{wxml,wxs,expression,asset,index.js} + logic/ + style/ + core/{env,npm-resolver,npm-builder,sourcemap,compatibility,compatibility-reference,renderers,expression-parser} + pipeline/{build-pipeline,compile-target,compile-stages,config-compiler,stage-channel,publish}；原平铺文件消失 | P-CL01 + 目录 ls 对照 README 结构图 | pending |
| A-CL1 | R-CL1 | 61 函数穷举迁移完成：新位置函数计数合计 61；view-compiler.js 不存在；每函数体逐字不变（可抽样 diff） | P-CL02 | pending |
| A-CL2 | R-CL2 | WORKER_ENTRY 映射生效（三域 spawn 路径正确）；12 spec import 更新完成 | P-CL03 + 全量 vitest | pending |
| A-CL3 | R-CL3 | 全量 vitest 绿（559+）；nomap + sourcemap diff=0；fe/packages 零触碰 | P-CL04 / P-CL05 | pending |
| A-CL4 | R-CL4 | diff 仅目录移动 + import 更新 + stage-channel 映射；无函数体变更 | P-CL06 | pending |
