...
| --- | --- | --- | --- | --- |
| A-WR0 | R-WR0 | grep 零调度残留（isMainThread/parentPort/collectOutput/outputCount/if(!isMainThread)）in view/logic/style/output/compatibility | `grep -rn "isMainThread\|parentPort\|collectOutput\|let outputCount\|if (!isMainThread)" src/compiler/{view,logic,style}/index.js src/compiler/pipeline/output.js src/compiler/core/compatibility.js` 退出码 1 | pending |
...
| A-WR6 | R-WR6 | pendingWarnings 归 logger（BufferingLogger）+ warnedItems 留模块级 + takeCompatibilityWarnings 废弃 | grep `pendingWarnings` 零 in compatibility.js + grep `warnedItems` 保留 + grep `logger.flush()` in runtime.js | pending |
...
| A-WR8 | R-WR8 | emitEntry async Promise<void>（无 return number）+ D-E-9 回流 architecture-notes | grep `return 1` 零 in emit.js + architecture-notes D-E-9 废弃标注 | pending |
| A-WR9 | R-WR9 | 测试直连用 abilityContext.run({ FileSink, ConsoleLogger }) 注入 | grep `abilityContext.run` in __tests__/ + 40 测试不再 parentPort null 崩溃 | pending |
| A-WR10 | R-WR10 | 4 组产物 diff=0 + vitest 584/584 + tsc build OK | diff -rq baseline current + vitest 输出 + tsc 输出 | pending |
