...
| A-WR7 | R-WR7 | executeTask 契约 6 点（含 result 形状，F35）+ new Worker only in executor.js | grep `new Worker` only in executor.js + `export function executeTask` 签名审查（6 点：输入/output/onOutput/onProgress/result 形状/资源层） | pending |
