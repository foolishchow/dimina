...
Status: **ready（2026-09-21）** — 伞级 MUST；子门细化自有 R-*。
...
- **stale node 清理**：watch `storeInfo` merge 不删 node（F-SIM-5 merge 可复活 stale node）；须补 `removeNode(id)`——`storeInfo` merge 后删旧 snapshot 中有但 `createInitialDependencyGraph()` 新建集中无的 **entry 型** node（page/component）。非 entry 模块节点保留。`merge()` 保持纯加法语义。
