...
- **R-PW-4** `view/index.ts` MUST 不含 `compileViewTree` / `compileModule` / `viewParseWalk` / `insertWxsToRenderResult` 定义（只 import + 编排 + `bindVueToolsLive` + `bindTransformOrchestrator` 注入 + re-export from `tools.ts`/`include.ts`）。
