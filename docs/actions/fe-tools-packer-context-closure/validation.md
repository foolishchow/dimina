...
| P-PC3 | orchestrate 北星签名 | orchestrate(ctx: PackerContext, state, options)——ctx 显式（非 ALS 派生）；buildPackerContext helper + OrchestrateCallOptions | ◐ PC-B10a done（signature）；implements + result→EmitEntry[] reconcile deferred（D-OR-7 三重张力：北星 return type + state type + 结构锚点，需北星 interface 演进） |
