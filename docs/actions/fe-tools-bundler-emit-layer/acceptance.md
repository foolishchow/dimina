...
| A-E5 | R-E5 | 消融有效；契约/output 回流 architecture-notes | 消融由行为 0 对拍（current=emit vs baseline=手写 diff=0）+ grep 锚定（拔 output.write→直写 grep 命中现已零命中）共同覆盖；契约/output 回流 architecture-notes 待 Close | ✅ pass |
