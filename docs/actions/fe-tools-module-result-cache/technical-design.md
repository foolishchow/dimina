...
- 缓存值类型：`CachedModuleResult = { compileInfo: CompileInfo, logicDependencies: string[] }`——`logicDependencies` = require/import dep ID 列表（transform AST walk 时捕获，永远新鲜；**非图查询**——图无 `removeDependency` API，stale edge 会导致多余模块入产物）。
- 形态：`class ModuleResultCache { get(moduleId): CachedModuleResult | undefined; set(moduleId, result: CachedModuleResult): void; has(moduleId): boolean; delete(moduleId): void; clear(dirtyIds: Iterable<string>): void; size: number; toJSON(): [string, CachedModuleResult][] }`
...
- **`logicDependencies` 捕获**：transform AST walk 中发现的 require/import dep ID（`logic/index.ts:265/287/312/337` `addDependency` 调用点）；在 `dependenciesToProcess` 收集时同步捕获全量列表，存入 `CachedModuleResult.logicDependencies`。
...
buildJSByPath(page) [worker: cache snapshot + dirtySet]:
  for each module in page:
    if snapshot.has(moduleId) && moduleId NOT in dirtySet:
      ...
      → dep 发现用 cached.logicDependencies          ← M2 新增（非 graph，避免 stale edge）
    else:
      → transform → compileRes.push(info)           ← 现有
      → AST walk 发现 require/import → 存 logicDependencies  ← M2 新增
...
| 资产 | 角色 |
| --- | --- |
| `model/dependency-graph.ts` | M1 `getInvalidatedModules` 已交付（M2 只消费）；M2 **不**用 `getDirectDependencies` 做 dep discovery（图无 removeDependency，stale edge 风险） |
...
| **D-RC-3** | worker 回填：**I** — cache snapshot（`CachedModuleResult[]`）经 IPC 传入 ephemeral worker；主线程管缓存实例；worker 只编译脏模块；**响应消息增加 `compileRes` + `logicDependencies` 字段**（协议变更）→ 主线程组装 `CachedModuleResult` 更新 cache（省 compute 非 IPC）；cache hit dep 发现用 `cached.logicDependencies`（非 graph，避免 stale edge） |
