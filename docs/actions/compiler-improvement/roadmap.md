...
| A3 | L2 CSS 热替换 + L3 模板热重挂（render/container-sdk dev-only 扩展） | `hmr-l2-l3` | A2；L3 需先过 RFC 假设 1 验证 | draft（2026-09-08）；假设 1 已条件通过（setupData 回放可行；需 A3 新增模块热替换 + 页面级 remount 原型，见 RFC §7.1）；边界定稿为 Web 容器 dev-only |
