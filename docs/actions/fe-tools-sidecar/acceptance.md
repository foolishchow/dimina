...
| A-004 | R-006 | parse→Document（或等价边界）有隔离测试 | vitest 路径 | **pending**（随 [`fe-tools-wxml-ir`](../_archive/complete/fe-tools-wxml-ir/README.md)；2026-09-14） |
| A-005 | R-007 | Document/parser 模块无 `platform ===` 分支依赖（抽检） | 代码审查记录 | **pending**（随 fe-tools-wxml-ir） |
...
| A-007 | R-009 / D-TS0-1 | 终态 B：`git diff origin/main...HEAD -- fe/packages` 为空（或 sync-rhythm §5 白名单未过期）；`dimina-cli` 冒烟仍通过 | [sync-rhythm.md](./sync-rhythm.md) §1 实测 + bootstrap validation | **pass（文档门）** — 2026-09-12 实测 diff 空；操作真源已冻 |
...
- **TS-1 前置已由** [`fe-tools-bootstrap-copy`](../_archive/complete/fe-tools-bootstrap-copy/README.md) **交付**：A-001 / A-003 / A-006 / A-009 / A-010 的证据见该 Action validation；伞表对应项仍可在伞闭合时统一勾选。**A-007** 终态 B 可检查句与同步节奏已于 TS-4（[sync-rhythm.md](./sync-rhythm.md)）最终化并记 **pass（文档门）**。**A-004 / A-005 属 TS-2**，由 [`fe-tools-wxml-ir`](../_archive/complete/fe-tools-wxml-ir/README.md) 承载（**`ready`**，未实施）。A-002 须持续保持（根 `fe` 对 `@dimina/bundler` 的 workspace 依赖不算 `packages/*` 反向依赖）。
- 「子门」措辞：TS-1 不立子门；TS-2 = `fe-tools-wxml-ir`；TS-4 并入伞文档，无独立 Action。
