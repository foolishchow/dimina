...
- **TS-1 前置已由** [`fe-tools-bootstrap-copy`](../_archive/complete/fe-tools-bootstrap-copy/README.md) **交付**：A-001 / A-003 / A-006 / A-009 / A-010 的证据见该 Action validation；伞表对应项仍可在伞闭合时统一勾选。**A-007** 终态 B 可检查句与同步节奏已于 TS-4（[sync-rhythm.md](./sync-rhythm.md)）最终化并记 **pass（文档门）**。**A-004 / A-005 属 TS-2，随门 deferred（2026-09-12）**，再激活见 README。A-002 须持续保持（根 `fe` 对 `@dimina/bundler` 的 workspace 依赖不算 `packages/*` 反向依赖）。
- 「子门」措辞：TS-1 不立子门；TS-2+ 可为独立 Action（TS-2 近端不 formalize）。TS-4 并入伞文档，无独立 Action。
