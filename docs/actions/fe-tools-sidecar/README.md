...
**近端替代重心**：[`fe-tools-project-store`](../_archive/complete/fe-tools-project-store/README.md)（**complete / 已归档**：PS1+PS2）+ [`fe-tools-build-pipeline`](../_archive/complete/fe-tools-build-pipeline/README.md)（**complete / 已归档**）；**session 唯一管会话**。（目录归置与 TS-4 已完成。）

### PS3 deferred（2026-09-12）

**决定**：不将 ProjectStore 的增量装载（`applyChanges`）与订阅（`subscribe`）作为近端主线。

**理由**：PS1+PS2 已完整支撑 build/watch/dev 三入口——watch 增量**编译**（plan 的 affectedEntries/stages）已交付，增量**装载**（applyChanges）属未来性能优化，当前 watch rebuild 采用全量 `store.load` + 旧图 merge（调度草案 **L2**：对齐今日 `storeInfo`）；preview 消费已由 `beforeBuild` ctx + lifecycle 事件覆盖（`setPendingReload` / `notifyBuildPublished` / `notifyBuildError`），**无 consumer 需要 store 订阅**。R-006/R-007 仍为伞级 MUST，**延后实施**而非取消目标。

**再激活条件**（满足其一即可重开讨论并另立 Action）：

1. watch rebuild 全量 load 成为可量化性能瓶颈（需要 `applyChanges` 增量图更新避免重扫）；
2. preview 出现需要 store 内部 metadata 的真实消费方（如依赖图详情展示、增量变更推送）。
