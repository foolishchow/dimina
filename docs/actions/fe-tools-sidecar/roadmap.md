...
| TS-3 | 可选抽出薄编排包；sdk 仅在 `@dimina/web-container-sdk` 内深改 | 可选未来 Action；控制面先行见独立 [`fe-tools-bundler-session`](../fe-tools-bundler-session/README.md)（`draft`；拆包更晚） | bootstrap-copy complete | pending / 可 deferred；**不阻塞本门** |
