...
Status: **ready（2026-09-14）** — D-CT 已拍板；升 `in_progress` 时记基线 SHA（随门递进）；T0/T1/T2 各自 PR，禁混（P5）
...
| 3 | 测例 | bundler-session.spec（或 compile-config spec）+ `build + platform:'web'` 拒绝用例（消息断言 `D-R2/C: command:'build' requires`）；**dev 侧既有用例不改** |
...
| 5 | 消融 | 拔 createCompileTarget 回落内联 → 结构锚定失败 → 恢复全绿 |

## T2 触达序（动态派生 · 行为 0）

| Step | 文件 | 动作 |
| --- | --- | --- |
| 1 | `src/compiler/compile-target.js` | + `readLoadBindings()`（env 唯一读取点）+ `deriveStagePlan(target, bindings)`（纯：最终 stages ∩ mini-game 过滤 / sourcemapTargetPath / stylePages 合成 / 三 stage workerOptions 完整包） |
...
| 5 | 消融 | 拔 deriveStagePlan（恢复闭包内联）→ 结构锚定失败 → 恢复全绿 |
