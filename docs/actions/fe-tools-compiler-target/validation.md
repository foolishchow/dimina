...
| P-CT05 | 结构判据 | ① compile-target.spec 单测（create：白名单 / 非法 stages / renderer 校验 / **消息文本锁定**；derive：bindings 组合矩阵）+ ② 源文件锚定（**锚定范围 = `'编译项目'` 闭包内**：`build-pipeline.js` 该闭包无 `getRenderer(` 反查、无 `!miniGame` 直耦 push、无内联 `sourcemapTargetPath`、无 `MODE_PRESETS`/`sourcemapStrategyFor` 私算、无手工三捆 workerOptions；`BUILD_END` 载荷 appId 经 bindings）③ E1 不变量文档 + grep 锚定 | A-CT1 / A-CT2 / A-CT4 / A-CT5 | pending |
