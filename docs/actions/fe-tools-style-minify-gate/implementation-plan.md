...
| 2a | `style/emit.ts`：加 `isDiffVerifyMode()`；`emitStyle` 激活 `minify` 参数（D-SM-1/3） | pending |
| 2b | `style/parse-walk.ts`：sourcemap=false 路径 minifyCss 调用加 gate（D-SM-2） | pending |
| 3 | P-SM* / A-SM*；回流 architecture-notes；close 另授 | pending |
