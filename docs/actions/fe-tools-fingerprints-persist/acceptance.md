...
| A-FP2 | R-FP-2 | `createWatchBuildPlan` 接收 `prevFingerprints`；tracked 文件 content hash 未变 → 从 `actuallyChanged` 过滤；`actuallyChanged` 用于 closure/stages/prepareNpm | P-FP02 | pending |
