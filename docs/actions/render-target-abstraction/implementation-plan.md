...
| ID | 任务 | 依赖 | 验证点 |
| --- | --- | --- | --- |
| P-001 | 冻结 target 类型、默认值、CLI/options 来源与未知 target 错误 | — | resolver unit + invalid target spec |
| P-002 | 新增 target registry/resolver 与 webview adapter 薄适配 | P-001 | adapter contract spec；既有编译函数仍为唯一实现 |
| P-003 | 将 view/style worker 阶段接入 resolver，不改变 worker 产物契约 | P-002 | compiler stage specs + default/explicit webview equivalence |
| P-004 | 可选内部 lifecycle target 诊断字段（若验证兼容），否则记录不增加 | P-003 | observer compatibility spec / decision record |
