...
compiler 支持 target 选择；未指定时默认 `webview`，显式 `webview` 与默认语义一致；未知 target 在编译开始前给出明确错误，不静默回退。

## R-002（MUST）view/style adapter 边界

view 与 style 编译通过 target adapter/接口调用。首个 `webview` adapter 复用现有实现，不复制或改变编译语义。
...
stage 生命周期可在内部诊断中标识 target；若增加字段，默认 webview 的公共/既有 payload 行为不被破坏。
